...
- **來源優先序（SoT precedence）**：SRS 由 PRD+舊系統+db-schema+refactor 多源合成；衝突裁準（refactor 限本層贏·不蓋 DB/Bible-PRD·DB-resolvable fact 不留 Pending·升級觸發→C 類）**內容權威＝`docs/spec-architecture.md §5b`**（決策 `docs/adr/ADR-0002-srs-sot-precedence.md`；本條薄殼指標）。
