...
- **下游 flow（RD/QA Agent Flow，2026-06-24）**：SRS Approved→code＝**RD flow**（`build-tasks/rd-codex-dispatch.md`／`rd-orchestrator-drain.md`、迴圈 `orchestration-playbook §5c/§6c`、軸 `§4c`＋`verifier-enforcement.toml`）；code→**三層測試 DB/BE/FE**＋測試報告清單＝**QA flow**（`build-tasks/qa-codex-dispatch.md`／`qa-orchestrator-drain.md`、迴圈 `§5d/§6d`、軸 `§4d`、FE `specs/fe-test-convention.md`、報告 `specs/qa-report-format.md`）。殼↔playbook 迴圈不變式由 `scripts/check-prompt-parity.py` 驗（含 SRS/RD/QA 三軌）。**內容權威＝playbook（薄殼指標）**。
