...
5. **新舊 DB 差異 + 新 table schema**（`build-tasks/schema-diff-findings.md` 等多 md；新 schema 權威＝新 DB 實查/entity）＋ **既有重構 spec/決策**（`feature-inventory`/`refactor-audit/per-page-reinventory-matrix`/`decisions`/`pending-register`/`disbursement-*-escalations`）→ **reconcile，更動後需求顯式納入 SRS**（見 DoD「新舊 DB + 既有 spec reconcile」；dispatch 範本 `build-tasks/prd-to-srs-codex-dispatch.md` §5）。
...
- [ ] **（2026-06-17）新舊 DB + 既有 spec reconcile**：SRS 對 `build-tasks/schema-diff-findings.md`（新舊 DB 差異 + 新 table schema，多 md：另含 `legacy-schema-db-reverify-findings`、`legacy/db-schema-catalog`；新 schema 權威＝新 DB 實查/entity）+ 既有重構 spec/決策（`feature-inventory`/`refactor-audit/per-page-reinventory-matrix`/`decisions`/`pending-register`/`disbursement-*-escalations`）；**更動後需求以 `Rn`/`@PENDING` 顯式納入 SRS**（每條附來源〔schema-diff 行/decisions 列/parity finding `file:line`〕+ 三判 tag `process/legacy-parity-sop.md`），**不得靜默遺漏**；已裁決策（AUD-6 精度/A-5 KHR/T24 照舊/頁合併 CS/CU→CSU）當**約束、不重議**；PRD 與既有決策/DB 衝突→標 `@PENDING`。`schema.sql` 帶「舊欄→新欄 + change 類型 + 三判」對照。
- [ ] 模糊詞已量化（NFR 可量測）。
