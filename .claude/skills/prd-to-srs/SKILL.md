...
- [ ] 模糊詞已量化（NFR 可量測）。
- [ ] **已過 SRS N 軸驗證**（`orchestration-playbook §4b` 的 A–G 軸：綜合〔軸A＝`spec-reviewer`〕／as-is parity／錯誤碼承載〔含裸名〕／安全·授權／DB reconcile／金錢·精度·截斷／可測試性；≥6 正交、跨模型；risk-tier T1 全跑、低風險頁可 A+E+G），**全軸無未解 Blocker**（＝**SRS 定稿 blocking gate**；圖上 ⑦ 是 code 階段的 advisory LLM review，兩者不同）。**採納修正後要再審一輪**——修正可能引入新錯（B1 修法曾引入 checkPointMap 副作用，靠複審才抓）。
