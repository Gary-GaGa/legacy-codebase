...
5. **對比輸入（多 md，reconcile，更動後需求顯式納入 SRS）**：① 新 table schema snapshot＝**local `docs/db-diff/`**（index `table_name`；markdown 欄位表非 DDL；新 schema 權威；舊→新僅 change-hint〔`note`/`usage_status`〕、**非完整 diff**）② 70% baseline 重構 spec＝**local `docs/refactor-spec/`**（index funcId/`epl-*`、多版並存→取 latest；含 API 欄位表 maxlength/必要＝量化來源；無正式 Rn/QA；「更動需求」＝新 PRD ⟷ baseline latest 的 delta）③ 我方既有裁定/約束＝repo `decisions`/`pending-register`/`refactor-audit/per-page-reinventory-matrix`/`disbursement-*-escalations`/`process/legacy-parity-sop`。→ 見 DoD「reconcile」；結構已盤點 2026-06-17（**⚠️ 2026-06-18 兩夾改名 `db-schema`→`db-diff`／`refactor`→`refactor-spec` ＋納入更多 spec → 內部結構／role〔snapshot vs 完整 diff〕／檔數待 owner 重掃；下方「非完整 diff／70% baseline」等描述為改名前快照**）、抽取細則見 dispatch 範本 `build-tasks/prd-to-srs-codex-dispatch.md` §5。
...
- [ ] 涵蓋 **happy / error / edge**——**多分支 `Rn`（錯誤處理/NFR 等）的 happy/error/edge 各需 ≥1 case**；`gate⑤ ≥1 覆蓋 ≠ 全分支覆蓋`（qa 自承「僅…分支／未撰寫／RD 補」會被 gate⑤ partial-warn，破假綠；源 00800 R15 僅 rollback／R16 僅 transaction 之教訓）。
...
- [ ] **（2026-06-17）新舊 DB + 既有 spec reconcile**：SRS 對 ① **local `docs/db-diff/`**（新 table schema snapshot，index `table_name`；新 schema 權威；舊→新僅 change-hint、非完整 diff）② **local `docs/refactor-spec/`**（70% baseline 重構 spec，index funcId/`epl-*`、取 latest；API 欄位表 maxlength/必要＝量化來源；「更動需求」＝新 PRD ⟷ baseline latest 的 delta）③ repo 既有裁定/約束（`decisions`/`pending-register`/`refactor-audit/per-page-reinventory-matrix`/`disbursement-*-escalations`）；**更動後需求以 `Rn`/`@PENDING` 顯式納入 SRS**（每條附來源〔db-diff 行/refactor-spec 段/decisions 列〕+ 三判 tag `process/legacy-parity-sop.md`），**不得靜默遺漏**；已裁決策（AUD-6 精度/A-5 KHR/T24 照舊/頁合併 CS/CU→CSU）當**約束、不重議**；PRD 與既有決策/DB/refactor-spec 衝突→標 `@PENDING`。`schema.sql` 以新 snapshot 為權威 + change-hint（`note`/`transpose_method`/`usage_status`/`match_status`）標「重構新增/別名/已棄用」+ 三判；**完整 row-level 舊→新 diff 此資料夾無→對舊行為回 `legacy-parity-sop`**。
