...
> **結構單一出處（canonical）＝`docs/specs/srs/spec-template.md`**（複製它起手）。段標題用英文、不編號、同概念用 canonical 唯一名（DB 段＝`DB Reconcile / Delta`、待決段＝`@PENDING`…）。**新 bundle 須通過 `check-srs-bundle.py` 結構檢查（warn-clean）**；既有 14 包標準定版前產出、漂移為已知（grandfathered，不回頭改）。下列十段＝語意說明，落地格式以 template 為準。
