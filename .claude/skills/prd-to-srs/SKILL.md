...
5. **對比輸入（多 md，reconcile，更動後需求顯式納入 SRS）**：① 新舊 DB 差異 + 新 table schema＝**local `docs/db-schema/`**（owner 提供；新 schema 權威）② 70% 當初依據的重構 spec＝**local `docs/refactor/`**（owner 提供；「更動需求」＝新 PRD ⟷ 此 baseline 的 delta）③ 我方既有裁定/約束＝repo `decisions`/`pending-register`/`refactor-audit/per-page-reinventory-matrix`/`disbursement-*-escalations`/`process/legacy-parity-sop`。→ 見 DoD「reconcile」；dispatch 範本 `build-tasks/prd-to-srs-codex-dispatch.md` §5（①②結構待 owner 回報後補抽取細則）。
...
- [ ] **（2026-06-17）新舊 DB + 既有 spec reconcile**：SRS 對 ① **local `docs/db-schema/`**（新舊 DB 差異 + 新 table schema；新 schema 權威）② **local `docs/refactor/`**（70% 當初依據的重構 spec；「更動需求」＝新 PRD ⟷ 此 baseline 的 delta）③ repo 既有裁定/約束（`decisions`/`pending-register`/`refactor-audit/per-page-reinventory-matrix`/`disbursement-*-escalations`）；**更動後需求以 `Rn`/`@PENDING` 顯式納入 SRS**（每條附來源〔db-schema 行/refactor spec 段/decisions 列〕+ 三判 tag `process/legacy-parity-sop.md`），**不得靜默遺漏**；已裁決策（AUD-6 精度/A-5 KHR/T24 照舊/頁合併 CS/CU→CSU）當**約束、不重議**；PRD 與既有決策/DB/refactor-spec 衝突→標 `@PENDING`。`schema.sql` 帶「舊欄→新欄 + change 類型 + 三判」對照。
- [ ] 模糊詞已量化（NFR 可量測）。
