...
啟發式限制（誠實標示，同 verify-c0 的煞車條款）：
  ...
  - gateⒷ 為 funcId 檔名 glob + `(REQ|FR)-\\d{3}` token 比對（容 PRD 用 FR-### 或 REQ-###、以 PRD 為主）；快照/trace 不存在時整段 advisory
  ...
  - gateⓇ＝**段標題啟發式**：只認 spec.md `^#{1,6}\\s` 標題行含關鍵字（新舊 DB/DB 對照/對照表/
    更動 delta/delta 清單/diff/reconcile）；**只驗段存在、不驗內容**——空標題/否定段（『本期不做』）
    亦 PASS（內容真確＝spec-reviewer 紅旗⑥）；縮排標題/`#`後無空格/內文提及不算（warn 級、非 FAIL）。
...
REQ_RE = re.compile(r"(?:REQ|FR)-\d{3}")  # 容 PRD 用 FR-### 或 REQ-###（以 PRD 編號為主；2026-06-18 EPROC00118 用 FR-###）
...
RECONCILE_HEAD_RE = re.compile(
    r"新舊\s*DB|DB\s*對照|對照表|更動\s*delta|delta\s*清單|diff|reconcile", re.I)
