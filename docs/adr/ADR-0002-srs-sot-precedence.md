# ADR-0002：SRS 來源優先序（PRD/舊系統/db-diff/refactor-spec 多源合成的裁準）
...
## Context
- SRS 現由 **PRD + 舊系統 + db-diff + refactor-spec** 四源合成，來源**會不一致**：
...
## Decision
1. **來源優先序梯（依「問的是什麼」選權威）**：業務意圖 `Bible>PRD`；需求驗收 `PRD`；FE/API 行為與契約 `refactor(latest)>legacy`；物理結構 `new DB snapshot>refactor-spec doc>legacy DDL`；既有資料/字典 `new DB query`。
2. **Rule 1（DB-resolvable fact 不留 Pending）**：可由 `docs/db-diff/` 判定的 fact 直接撈出寫入、不佔 `@PENDING`；**三護欄**＝只解 fact 非 policy、強制 provenance（source＋snapshot date＋decision 列）、與 PRD/Bible 矛盾則升級；讀不到 snapshot 則退回 `@PENDING`。
3. **Rule 2（refactor 限本層優先）**：refactor 只在 FE/API 契約層贏（留 `REF-Dn` delta），**不蓋** db-diff 物理 / Bible-PRD 意圖。
...
- － DB-resolvable 需「轉換讀得到 db-diff」；目前 spec repo 無此層（在母資料夾）→ 讀不到時仍退回 Pending（前提已寫入 Rule 1）。
