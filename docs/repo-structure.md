...
- **live**（還要用）：`00800-verification-findings.md`（SRS as-is 來源）、`00300-return-recon.md`/`c0-authz-sql.md`（待派工）、`schema-diff-findings.md`（schema SSOT）、`full-refactor-audit.md`+`refactor-audit/`（✅ 完成 06-11；歸檔待 AUD 收口）、`phase-g-csu-mainflow-fe.md`（企金 FE 補建）、audit 修復包（✅×3 已歸檔；餘 `00640-pdf-export-fix`）、`bible-gap-recon.md`、`a1-funcGetExchangeRate-spec.md`、`phase-d-a1-exchange-stub-investigation.md`；✅ `00800-rimat-fix` F1–F9 落地歸檔（06-12）。
