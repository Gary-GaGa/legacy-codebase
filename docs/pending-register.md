...
| **T24 SRS re-open**（`EPROISU0922`）🟡 **partial（06-22 narrowed）** | 主體欄已逐欄折回 `TBD-0922-001`（v1.5 為主源；delimiter/CRLF/encoding、null/default、A16、A31/G7、G11/G12、C12/C13/C20/C26/A52、E21、G4/G10/H8）✅。06-22 residual pass closes A15 blank-preservation and closes segments `B`/`D`/`H1-H7` under `TBD-0922-008`; the only remaining open item is `TBD-0922-007` for `T24_COMPANY` feeding T24 `B8`/`C9`. DB-live schema-diff resolves the DB fact that `TB_BRANCH_PROFILE.T24_COMPANY` exists, and the current native-map backend path already reads that key; the generated db-diff snapshot now carries a correction note because it is stale/omissive, and current `TBBranchProfileEntity` still does not map the column. Keep pending only for RD entity/schema-model alignment confirmation and owner signoff that no separate T24 contract override is required. | 🟡 partial（殘 `TBD-0922-007` / B8-C9）| 撥貸 domain＋RD/Codex | 06-20→06-22 | `docs/specs/srs/EPROISU0922/spec.md` `REF-D8` / `TBD-0922-007` / `TBD-0922-008`; `docs/build-tasks/schema-diff-findings.md:246`-`249`; `docs/db-diff/02_tables/TB_BRANCH_PROFILE.md:47`-`50`; `decisions.md` 06-22 residual closeout |
