...
| EPROZ00100 / EPROC00118 SRS @PENDING → **contract 軸 RD/DBA 實作 gap** | **2026-06-20 owner-decision @PENDING 全關**（00100 `PENDING-Z001`~`Z010`、00118 `PENDING-001`~`018` 全裁；bundle 內**無 open @PENDING**、`gateⓅ` PASS；機械閘門兩頁 exit 0、本環境 pyyaml 深驗補跑 PASS）→ 殘 Approved-blocker **改為 contract 軸**：FE/BE 實作（DTO/status/`TB_API_AUTH` seed/save guard）未追上 to-be SRS（00118 四端點 c0 row 缺、`epl-info` 缺 `mainBorrowerName/checkpointStatus`、`parseScore` 吞 NFE→`0`、`getScoreDatetime` 無 `ZoneId`、save 無 `!isAO&&!isCR` reject）＝**implementation conformance、非裁定缺**；不以改回現況掩蓋。**部署最硬＝00118 四 `TB_API_AUTH` seed＋非授權 save reject 證據（before any testing deployment）。** **spec-reviewer post-fix 複審（opus×2, 06-20）兩頁無 Blocker＝spec 側 Approved-gating 語意關已過**；工單＝`contract-closeout-card`。**▶ 進度（06-20 RD 母資料夾回報，未獨立碼驗）：8/13 code 落地（compile/ng build pass）**——00118 save guard/epl-info/parseScore/scoreDatetime/CUR_RATIO ＋ 00100 reasonList/6 月 guard/docNo1 已實作；**剩＝A1/B1 `TB_API_AUTH` seed apply（DBA）＋B2/B3/B4 init/download-token/session-bridge（下一張 RD，repo 無安全 provider）＋各項 acceptance/regression 測試證據** → **contract 軸尚未全 PASS、未升 Approved**。| 待 DBA seed apply＋RD（B2/B3/B4）＋測試證據（非裁定）| RD/DBA | 06-20 | **工單 `build-tasks/EPROZ00100-EPROC00118-contract-closeout-card.md` §進度**＋`pilot-srs-pending-verification.md`+`.sql`＋`c0-authz-sql-findings.md`＋各頁 `spec.md`（已無 open @PENDING）|
