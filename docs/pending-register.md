...
| **T24 SRS re-open**（`EPROISU0922`）| 〔06-20 owner re-open，supersede 06-16「T24 照舊」〕產 0922 T24 SRS 時不沿用「照舊」為定案 → **SRS 逐欄/逐段（A–H）重推 to-be、owner 逐欄 confirm**；未 confirm 欄＝`@PENDING`（金錢/交易，ADR-0002 升級觸發）；原 parity-fix `3d6f446`＝as-is 證據 | `EPROISU0922` T24 段 A–H 各欄 to-be 定版 | 撥貸 domain（逐欄 confirm）＋ RD/Codex（as-is 逐欄坐實 `file:line`）| 06-20 | `decisions.md`「T24 於 SRS 層 re-open」列＋`build-tasks/prd-drafts/PRD-CDC-EPRO-0001-EPROISU0922-v0.1-DRAFT.md` REQ-004 |
| **0921 A-4/M6 SRS re-open**（`EPROISU0921`）| 〔06-20 owner re-open，supersede 06-17「A-4/M6 照舊」〕**A-4 檢核（REQ-002/006）＋ M6 完工日（REQ-003）逐項重推 to-be、owner 逐項 confirm**；未 confirm＝`@PENDING`（金錢/檢核 ADR-0002）；已修 code＝as-is 證據（confirm 後可維持＝舊 或 改）；regression REQ-004/005 維持 | A-4：CO_CHECK 判定／mbCheck gate／law firm IS_SHOW／address UPD_DATE／DATA_SEQ 各項 to-be 定版；M6：兩欄值來源定版 | 撥貸 domain（逐項 confirm）＋ RD/Codex（as-is 坐實／trace）| 06-20 | `decisions.md`「0921 A-4/M6 於 SRS 層 re-open」列＋`build-tasks/prd-drafts/PRD-CDC-EPRO-0001-EPROISU0921-v0.1-DRAFT.md` REQ-002/003/006 |
