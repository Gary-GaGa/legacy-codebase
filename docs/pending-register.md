...
| **EPROC00118 TBD**（新 SRS v0.1，企金 scorecard pilot）| 10 PRD TBD：001 param seed／**002 `CR_SCORE_CARD_COMPLETED` 兩碼+CreditEval 覆寫（與 E1/E2 合流）**／003 後端強檢／004 role 003 可否編／005 AO copy `_AR_SCR`/`_INVENTORY_SCR`／008 default toggle UI／009 有效日期納 `END_DATE`／010 AO→CR default 同步／**AUTH `TB_API_AUTH`/`TB_ROLE_TASK` seed**／ERR-STATUS 錯誤碼→HTTP status | `00118` SRS 升 Approved（**TBD-002 與 c0 E1/E2 合流**）| PM/SA/RD/風控（分項見 spec）| 06-18 | **單一出處＝`specs/srs/EPROC00118/spec.md §@PENDING`**（pilot、機械閘門 PASS、N 軸 A–G 無 Blocker；TBD-006/007 已 closed/non-goal）|
