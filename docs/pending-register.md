...
| EPROZ00100 / EPROC00118 SRS @PENDING | **2026-06-18 重產 in-review、06-19 修正中**：兩頁 spec §@PENDING 為單一出處（00100 `PENDING-Z001`~`Z010`、00118 `PENDING-001`~`018`）；`gateⓅ` PASS；跨模型 N 軸 Blocker/Should-fix 已回補至 bundle，殘留 owner/RD/DBA 待裁以各頁 `spec.md §@PENDING` 為準。**決策分組/分層 aid＝`build-tasks/EPROZ00100-EPROC00118-pending-decision-brief.md`**（先裁哪 2 條解 Approved、哪些可一次共決、哪些是 DB-resolvable；非 SSOT、指回 spec.md）| 待人審/裁/修 | PM/SA/RD/風控 | 06-19 | 各頁 `spec.md §@PENDING`（單一出處）＋決策 digest |
