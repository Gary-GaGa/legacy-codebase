...
| **AUD-10**（F-OWN-1，owner 盤點 reconcile）| **批次層 `EPROZ0_B001–B008` 完全未當工作單位追蹤**——owner 權威盤點列 8 批次，我們 inventory/diff 全無；**碼驗回（06-16）**：✅ B006/B007 FOUND（撥貸下游交付不缺）；🔴 **B005 匯率排程 app 碼缺**（待 domain 判 inline-A1-取代 vs 漏建→查 `TB_EXCHANGE_RATE` 非-authorize 讀者）；⚪ B008→ops；❓ B001–B004 待完整 findings | 撥貸端到端下游＋B005 排程 | 撥貸 domain（B005）/ops（B008）/RD（B001-4）| 06-16 | findings `build-tasks/aud10-batch-layer-reverify-findings.md` |
