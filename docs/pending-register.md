...
| ~~**EPROC00117 SRS in-review**~~ ✅ Approved 2026-06-24 | Open SRS @PENDING IDs: none → owner-stamped 規格定版 Approved after axis A–F independent confirmation (SaveRequest DSR fields + fixed USD closed). 企金線老系統 parity 補比 (row 26) remains a separate downstream track. | — | owner | 2026-06-24 | `docs/specs/srs/EPROC00117/spec.md` Status |
| **EPROC00115 SRS ready-for-owner-finalization** 2026-06-24 | Open SRS @PENDING IDs: none（12 finalize decisions closed by owner 2026-06-24：LOAN_LIMIT_TYPE 保留 current 語義、第三幣別只收 USD/KHR、row 上限、blank-flag、RC/0215 checkpoint、RC 舊案 Finished、報表幣別修正、facility label、route/menu、facilities `@NotNull`、RP20 雙層授權、孤兒錯誤碼綁定）。**獨立 axis A–F 複審 🟡（未蓋章）**：openapi 金額 pattern `\\.` 雙跳脫致 regex 語義錯、R5/R11/R16 bug-fix 缺 `REF-Dn` delta、R16 報表 cross-module 變更未掛 R9 條款／未評 CS0181/CU0181——待 dev 修＋再審後 owner 蓋章。 | Owner finalization checkpoint | owner | 2026-06-24 | `docs/specs/srs/EPROC00115/spec.md` Decision Register |
| ~~**EPROC00119 SRS in-review**~~ ✅ ready for owner finalization 2026-06-24 | Open SRS @PENDING IDs: none. RP40-RP54 closed by owner 2026-06-24 finalize decisions: RP40 code-as-baseline with legacy `EPROC0_0119`/`EPROC0_0219` plus current corporate code at commit `3dfebcf`; RP43/RP50/RP52 policy-2 bug fixes; RP44 AND eligibility; RP51 dual authorization; remaining RP items converted to SRS to-be requirements with RD/DBA conformance work called out separately. **獨立 axis A–F 複審 🟡（未蓋章）**：RP43 balance-vs-cash 比較方向未量化釘死（policy-2 修向待 SA/owner 定 equal vs not）、code-as-baseline 行級引用缺 `@SHA`（Rule 1c 格式）、first-year opening balance DTO 收值假設未驗（TBD-010）——待 dev 修＋再審後蓋章。 | Owner finalization checkpoint | owner | 2026-06-24 | `docs/specs/srs/EPROC00119/spec.md` Decision Register |
| **EPROC00120 SRS ready-for-owner-finalization** 2026-06-24 | Open SRS @PENDING IDs: none（P-001..P-013 closed by owner 2026-06-24：P-009 code-as-baseline〔legacy 0120/0220 + current @`3dfebcf`〕、P-011 雙層授權、P-012 parent checkpoint 獨立 endpoint、P-013 HALF_DOWN、P-001 STRICT reject、P-002 PERIODS=0→N/A、P-005 Finished 必填集、P-006 RC 舊案照 legacy、P-007 i18n `EPROC00120_FUNC_NAME`、其餘採 agent 建議）。**獨立 axis A–F 複審 🟡（未蓋章）**：db-diff 當硬權威但缺「規劃 repo 不可達／已於母資料夾覆核」disclaim（與 P-009 不對稱）、R1/R19 恐因 P-012 誤排本頁自有 `APPLICATION_NAME`/checkpoint 值（FE 斷料）、P-005 派工卡措辭「自動公式欄」與 spec（PRD 6 人工必填欄）不一致——待 dev 修＋再審後蓋章。 | Owner finalization checkpoint | owner | 2026-06-24 | `docs/specs/srs/EPROC00120/spec.md` Decision Register |
