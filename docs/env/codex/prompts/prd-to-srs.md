...
- `spec.md`（SRS 主檔）→ 閘門③｜`openapi.yaml`（真實 epl-* 契約）→ ①｜`schema.sql`（表/欄/約束）→ ②｜`qa-cases.md`（`covers: Rn`；**test-ready**：Given 可 seed/When 對 epl-*/Then 有 DB 驗證點，橋接 `docs/specs/qa-to-test.md`）→ ④⑤
