...
## 結論
- 2026-06-26 v1.1：RB-1/RB-2/RB-3 已在 harness/local-env runner 層修畢。
- LT-1～LT-5 已用 per-case role 單次 runner 全 PASS，API count 與 DB count 對齊。
- RI-1 已修正 SQLPlus temp SQL BOM 後 PASS，API row 與 DB row 逐欄一致。
- RI-2 已消歧：fixture 是「有 case、無 `TB_REVISED_ITEM` row」，API 回 success data envelope 但缺 openapi required `revisedType/revisedTypeSize`，已開 RD 契約缺口卡。
...
- `docs/verification/phase-v-api-selfverify-responses/RI-2-response.json`
- RD card：`docs/build-tasks/phase-v-ri2-query-reviseditem-contract-gap.md`

## v1.1 rerun（2026-06-26）
- Runner：`tools/phase-v-run.ps1 -SkipBuild`（`mvn clean` 重跑遇 Windows file lock；產品碼未改，沿用既有 build artifact 啟動 runtime）。
- BaseUrl：由 local-env descriptor `services.be.url` 供應，實跑為 `http://localhost:5500`。
- Serviceability smoke：`/epl-list-casedistribution`，HTTP 200 + `code=0000`。
- Roles：LT-1 role `101`；LT-2 role `405`；LT-3/LT-4/LT-5/RI-1/RI-2 role `403`。
- Teardown：`local-env down` 成功，down 後無 5500/4200 listener。

| id | category | status | result |
|---|---|---|---|
| LT-1 | - | PASS | `1=1=1` |
| LT-2 | - | PASS | `5=5=5` |
| LT-3 | - | PASS | `0=0=0` |
...
- Status 2026-06-26: fixed by per-case role in manifest + harness/runner role login. No verification role or DB DML was added.
...
- Status 2026-06-26: fixed. Temp SQL is written as UTF-8 no BOM; RI-1 now PASS.
...
- Status 2026-06-26: resolved to RD contract gap; see `docs/build-tasks/phase-v-ri2-query-reviseditem-contract-gap.md`.
