...
frontmatter），照其〔輸入 / 輸出三檔(spec/openapi/schema) / spec.md 結構(canonical 一檔兩半;見 spec-template) / SRS 鐵則 / Brownfield 鐵則 /
...
- spec.md **上半 Contract**（Metadata header〔Status 雙軸/Owner/Slug/版本/上游 PRD/as-is 來源〕、
  ...
  covers-prd: + 強制點 FE/BE/both + 末尾 `[ev→Rn]`；as-is/決策/provenance 不內嵌〕、NFR、硬界線）
