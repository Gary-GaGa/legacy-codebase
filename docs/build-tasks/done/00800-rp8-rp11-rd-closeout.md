...
> **Status: SUPERSEDED / CLOSED（2026-06-23）**——本卡寫於 06-17、針對 **v0.9 封存版**的 RP8/RP11 `@PENDING`。00800 其後已**重產為 Approved**，新 bundle 已把 **RP8 ✅ / RP11 ✅ closed**（owner/RD 裁定，見 `docs/specs/srs/EPROZ00800/spec.md:221`、`:224`、`:208`）。故本卡的 RD 收尾任務**已由重產承載、不再為 open 工作**，僅留作歷史。下方「仍開」敘述為 v0.9 時點原文，已不反映現況。
> **性質（v0.9 時點原文）**：兩個 `spec.md §@PENDING`（**仍開**）——皆需 **RD 對產品碼判斷**（判據等價、DTO 形狀），planning repo 解不了。**只 RD 能結。**
