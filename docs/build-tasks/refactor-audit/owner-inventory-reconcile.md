...
> **2026-06-28 回填**：新版 owner Excel `C:\Users\00596357\Downloads\function-review.xlsx` supersedes 本檔 F-OWN-4 舊表口徑；`查核總表` target 欄 `E137:E140` 為 merged cell，視覺值全是 `EPROCSU0160`，故 `EPROCU0160` 不再是獨立新頁 target。

## 1. 口徑／總量（owner 表權威）
| 類別 | 數 | 說明 |
|---|---|---|
| **distinct 新頁（對應重構系統頁籤；工作單位）** | **67** | i0 11・ISU 22・c0 9・CSU 9・Z0 16（含 `EPROZ00670`） |
...
| `EPROCSU0110…0173` | 9 | §1 企金主流程（Phase G 收口；`EPROCSU0160` 含 `EPROCU_0160/0260`）| ✅ 覆蓋 |
| `EPROZ00100…00800` + `EPROZ00670` | 16 | §1 共用（z0）| ✅ 覆蓋 |
> 頁級覆蓋＝**67/67 對得上**（2026-06-28 owner Excel merged-cell target recheck）。**惟「碼在 vs 碼在疑未完」仍需 Codex 碼驗**（本檔 docs-only 不裁）。
...
### ✅ F-OWN-4 — `EPROCU0160` 對應不一致已由新版 owner Excel 收口（AUD-11 併 vs 分關案）
舊 owner 表曾顯示 `EPROCU_0160/0260` → **`EPROCU0160`（自成新頁）**，與我們 inventory 併為 `EPROCSU0160` 不一致。2026-06-28 改以新版 owner Excel `function-review.xlsx` 為準：`查核總表` target 欄 `E137:E140` merged cell 視覺值全是 `EPROCSU0160`，涵蓋 `EPROCS_0160/0260` + `EPROCU_0160/0260`。三判＝(b) 刻意演進·縮編 + (c) DB 結構差，keep 新 `EPROCSU0160`，`EPROCU0160` 不列新 target。
> **仍保留的實作風險**：06-17 碼驗指出 `CsuLoanConditionServiceImpl:597` 讀 `EPROISU0160`，而 CS/CU checkpoint 新表欄位＝`EPROCSU0160`；這是獨立 checkpoint potential regression，併入企金線 parity 補比處理，不再卡 F-OWN-4 的「併 vs 分」裁決。
...
- **(a) inventory ✅；F-OWN-4 已收口**：`EPROCSU0160` 併 `EPROCU_0160/0260`，owner Excel 2026-06-28 坐實；`:597` checkpoint key 另併 parity 補比。
...
3. ~~**CU0160 碼驗**~~ ✅：新版 owner Excel + feature-inventory §1A 已裁定併入 `EPROCSU0160`；不再分列 `EPROCU0160`。
...
- ~~**CU0160**~~ ✅：新系統企金無擔 Loan Condition 新 target 由 owner Excel 2026-06-28 坐實併入 `EPROCSU0160`。`:597` ISU0160 checkpoint key 另列企金線 parity potential regression。
