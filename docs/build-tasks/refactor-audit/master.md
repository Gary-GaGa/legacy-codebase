...
| S4 | M4 `cs` + M5 `cu` | `M4-cs.md`, `M5-cu.md` | 完成 |  | M4 total 20; M5 total 17. No breakpoint. CS/CU merged in new pages; list source differences separately. |
...
Recommended next session: S5. Read only the task card sections 0, 2, and S5; this `master.md`; and the relevant `page-mapping.md` hints. Then recount M6a `i0` from old source and dispatcher/web.xml before checking new FE/BE code.
