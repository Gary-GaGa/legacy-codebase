...
- Statements: 2 set-based Oracle `INSERT ... WITH ... SELECT` statements plus `COMMIT`
- Logical mappings: 29 `TB_API_AUTH` endpoint mappings + 8 `TB_ROLE_TASK` page mappings
- DB precheck scope: `epro-db/new.cmd`, SELECT only, `OVSLXLON02` owner/schema, no INSERT executed
- Expected `TB_API_AUTH` inserts now: 13. Current c0 rows: 15/29. Source i0 rows: 27/29.
...
### 00117 Check-Only Verification

| Object | Key | Exists? | Count | Roles / tasks | SQL action |
|---|---|---|---:|---|---|
| TB_API_AUTH | `epl-info-c0-financial-business` | N | 0 |  | check-only, no INSERT generated |
| TB_API_AUTH | `epl-save-c0-financial-business` | N | 0 |  | check-only, no INSERT generated |
| TB_API_AUTH | `epl-sele-c0-financial-list` | N | 0 |  | check-only, no INSERT generated |
| TB_ROLE_TASK | `EPROC00117` | Y | 1 | `E:001,002,403:信用調查_Credit Investigation_Financial Evaluation Table_GI(business owner)` | no INSERT generated |

### Apply Impact Summary

- `TB_API_AUTH`: 13 rows expected to insert; 15 target endpoints already exist and will be skipped; 1 target endpoint remains missing because its i0 source auth row is absent.
- `TB_ROLE_TASK`: 0 rows expected to insert; all 8 target pages already have the source-equivalent task row.
- `00117`: `TB_ROLE_TASK` exists, but all 3 current `TB_API_AUTH` endpoint rows are absent. This remains check-only per task scope and should be resolved by ops/owner if those endpoints are still expected to be callable under API auth.

### Ops Apply Checklist

1. Execute/apply only against schema `OVSLXLON02`; confirm current schema before applying and do not run against `OVSLXLON01`.
2. Idempotency check: first apply should insert 13 `TB_API_AUTH` rows and 0 `TB_ROLE_TASK` rows from the current DB state; a second apply should insert 0 rows because `TB_API_AUTH` is guarded by `API_ID` and `TB_ROLE_TASK` by `PAGE_CODE,FUNCTION`.
3. Resolve the missing-source list before Phase V signoff: `epl-pxls-c0-financial-statement-comments` has no c0 row and no i0 source auth row, and `00117` API auth rows are absent but intentionally outside this SQL's INSERT scope.
...
- `00117` current c0 API auth rows are absent in DB, but remain check-only / no INSERT generated by this task.
...
- `00117`: current c0 routes exist at `CsuFinancialStaffController.java:29`, `:36`, `:43`; DB verified `TB_ROLE_TASK.EPROC00117` exists, but the three current c0 API auth rows are absent. Per task, no SQL was generated for 00117.
