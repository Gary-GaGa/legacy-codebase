...
> **元件化（owner 定）**：下列鐵則抽成**通用、可選的 AI 流程工具**——本機 app 環境生命週期管理器（`tools/local-env.ps1` up/down/status），**與測試解耦、不綁 Phase V**。通用契約權威＝**`docs/process/local-env-manager.md`**；Phase V 消費見 `phase-v-env-manager.md`。本節＝該工具的鐵則來源；harness 改為「消費已起好的環境」（拿 `BaseUrl`、不起停）。
