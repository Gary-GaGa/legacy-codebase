...
| **owner 權威盤點表 reconcile**（補跑 2026-06-16；`SOURCE=docs-only`）| —（唯讀 diff）| ✅ PASS | owner 給最新權威盤點（197 列，存 `legacy/legacy-function-inventory.md`）→ zero-based 重推：**67 distinct 新頁 + 8 批次 + 00670 + EPROZZ_0100 API + 5 已無使用 + 53 fragment**。頁級覆蓋 **66/67 對得上**（家族全在）。**6 發現**：🔴 **F-OWN-1＝8 批次 B001–B008 完全未當工作單位追蹤**（feature-inventory/diff 全空；B005 匯率/B006 async/B007 SFTP 撥貸下游關鍵）→ 最大漏列、需 Codex 碼驗新系統批次等價物;🟢 F-OWN-2＝Property Info 家族 owner 標**已無使用**→ **AUD-1 可結**;🟢 F-OWN-3＝坐實 AUD-5（00670 舊無源）+AUD-4（demo 不遷）;🟡 F-OWN-4＝`EPROCU0160` owner 分出獨立頁 vs 我們併 CSU0160→碼驗;🟡 F-OWN-5＝AUD-2 佐證（0211/0213 流程無對應頁籤）;口徑：owner 67 頁級 ＝權威 target、我們 166 ＝細粒度展開（不矛盾）。輸出＝`refactor-audit/owner-inventory-reconcile.md`（§5 建議回填、§6 Codex 碼驗派工）|
