...
| `phase-g` G3 | **product hash 待補** | ✅ PASS（條件）| 鐵則 1 兩款手法各用對地方:**return dialog `endpointConfig` 參數化＋cancel popup CSU 變體**;Phase V 清單詳實且自抓兩個缺口（CSU download route 無——勿猜 endpoint、CSU 報表沿用 `epl-ppdf-isu-credit-proposal-report`）;**待補三樣:product commit hash、ng build 結果、ISU return dialog 回歸自測聲明**——補齊前 G3 不算審結,G4 可先行;另全卡加 Phase V 共通項=G1–G6 對 XD 設計走查 |
