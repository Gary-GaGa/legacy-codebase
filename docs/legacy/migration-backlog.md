...
- [ ] **新 DB schema 批次抽 entity**（`db-schema-catalog.md` §3）：✅ Phase 1 三表完成 → **B1 基礎建設**（`TB_PAGE_MENU`/`TB_APP_NO_SEQ`/`TB_FUNCTION_AUTH`…）→ **B2 借款人/擔保品** → **B3 i0/c0 財報/評分/CBC**（✅ 確認在同 Excel）。HOME 會截斷 → 用指定表名抽
