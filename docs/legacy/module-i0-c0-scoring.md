...
- **Scorecard**：`TB_SCORE_CARD_PARAM_DETAIL`（⚠️ 2026-06-16 DB 校正：`_MAIN`/`_SUB` 舊有新無，OVSLXLON02 僅 `_DETAIL`＝連動 AUD-7）、`TB_MAIN_BORROWER_PERSONAL/WORK/FAMILY_INFO`、`TB_COLL_ASS`、`TB_CORP_SCRCARD`。
...
- CBC 是外部信用/聯徵資料的**資料接入與維護**；repo 內透過本地 `EPRO_TB_CBC_*` 表讀寫（VO 類名；06-12 實查舊庫實表＝`TB_CBC_*`），未見獨立報表軌。
