...
- **匯出**：PDF / Excel 只能從**已存完整資料**產生、**不靠未存 UI 狀態**；缺任一表 → `FAILED_E116`（`R8`）。
...
2. **參考複製** `R2`：依現行 repository/案件日期資格（案件進度碼 + 合約型態）+ 三表完整才回；不符 → `FAILED_E116`、**不寫本案/checkpoint**。
3. **主檔 + 5 段重點** `R3`：主檔欄 + 5 段 highlight（API 各上限 6000 字、落地拆 `_1`/`_2` 各 3000）；raw legacy `HIGHLIGHT` 進 `HIGHLIGHT_CLOB` 當 provenance。**KHR 幣別金額無小數**、USD 保留小數（BR-016）。
4. **計算三表** `R4`/`R5`/`R6`：後端算衍生欄（確切公式/欄名見 spec）。**衍生欄 BE 算、不信 client 送**；保留 DB 實體 typo 欄名（如 `IMCOME_DATE`/`SHARDHOLDERS_LOAN`），對外用 canonical 名（BE 負責 mapping）。
...
**橫切（每階段都適用）**
- **錯誤/envelope** `R10`：業務錯走 HTTP 200 envelope、授權失敗走 401；`E130` 只從驗證分支回；錯誤碼/別名見 spec。
- **授權** `R10`（雙層）：平台 `TB_API_AUTH` seed + **後端 service case/page guard**（讀/編權限、存前先驗）；**只設 seed 不夠**。`epl-pxls` seed `ROLE` 比照 c0 ppdf。
- **下游/邊界** `R9`：保留 GI 下游 schema / 列序 / `DATA_SEQ` / checkpoint routing；FIX 不重建。
...
主檔＝財報主表（含 5 段拆欄 + `HIGHLIGHT_CLOB`）；三明細＝Balance / Income / Cashflow GI（PK `APPLICATION_NO, DATA_SEQ`）；另寫案件 checkpoint 表（CS / CU 依屬性 routing）；授權＝`TB_API_AUTH`（pxls seed 待補）。
...
**實作踩坑**
- **雙層授權**：只設 `TB_API_AUTH` 不夠，service case/page guard 必做（存前先驗）；`epl-pxls` 的 seed 待補（`ROLE` 比照 c0 ppdf）。
- **save-list 驗證**：三表陣列必非空、等長、無 null、1–5 列，且驗在 mutation 之前。
- **delete-and-rebuild**：少送的年度會被刪——這是預期行為，別誤判資料遺失。
- **DB typo 欄名**：實體 typo 欄名保留於落地（`IMCOME_DATE`/`IMPAIMENT_LOSS`/`SHARDHOLDERS_LOAN`/`AP_RELATED_PARITIES`），對外 canonical 名靠 BE mapping。
