...
| QA-014 | R9 | execute body item 帶非 Y/N（如空字串/"X")/ 送出 / **BE 拒絕**（400/`COMMON_MSG_ERROR_LON`，與 openapi `ItemFlag` enum=[Y,N] 一致）、DB 不寫入；blank→N 正規化責任在 FE（PRD §5.3.2）、非 BE | `TB_REVISED_ITEM.ITEMn ∈ {Y,N}`、無新 insert |
...
| QA-023 `@PENDING RP8` | R6,R7 | （placeholder，RD 確認等價性後撰寫）構造 `secureAttribute≠'U'` 但 `isCU=true`（或反之）、`isEdit` 與 `canEditList/isShowList` 分歧之屬性組合 / 進頁 / 行為依 to-be 判據（`isCU`/`isEdit`）而非 as-is 判據 | 兩判據分歧情境下 ITEM3/唯讀行為正確 |
