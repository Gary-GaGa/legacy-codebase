...
- **R13.4 RI-MAT-004 ⏸ @PENDING RP4**：**當** ITEM1/4~11 特定組合變更時，系統應刪 `LOAN_CONDITION_DETAIL`+`REVISED_ITEM_DETAIL`，並將相關 page menu 標重處理。as-is ⚠️：page menu 標 0110~0160（非 §5.6 清單，見 R14）。
- **R13.5 RI-MAT-005 ⏸ @PENDING RP4**：**當** ITEM1/4~11 由 Y→N 時，系統應從原始 detail 還原欄位（§5.5.2 矩陣）並更新 `REVISED_ITEM_DETAIL`。as-is ⚠️：ITEM10/11 else ITEM1 疑錯欄（＝RP4 命題、判據待 RP6 取數）、受 isNotSame gate（gate 移除已隨 RP1=A 定版，見 R11）。
...
| REQ-005 | R13.4,R13.5（@PENDING RP4）| QA-009（@PENDING RP4）|
...
- **本輪解鎖（RP1=A，可實作）**：R11 移除 isNotSame gate；R13.1 還原傳空值修復；R13.2 補「reference 無 guarantor」判斷；R13.3 補 `IS_ANY_COLLATERAL_PROVIDER` 更新；R5 補 disabled（含非 03 且 DB=Y 不可編）；R16 execute audit 側效異動摘要；UI `Finshed`→`Finished`（RP7）；**R14 checkpoint 欄位寫入對齊 DDL（RP10 已關 06-12 → rimat F8）**。
- **仍待**：R13.4/13.5（RP4，連動 RP6 取數）、init-query method（RP9）、execute DTO 形狀（RP11）、R6/R7 判據等價性（RP8）。
