...
2. **計算評分** `R3`/`R8`：依參數表（依申請日取**有效版本**）算每項 score → total → risk level + scoreDatetime。`scoreDatetime` 用 **UTC+8（Asia/Taipei）顯式時鐘**，非 JVM 預設時區。參數 `SCORE` 是字串，**非數字 = fail-fast 報錯**、不可偷當 0。calc **禁寫 DB**。
3. **Default 處理** `R4`：逾期 90+ 天 → 該角色 `Default` / score `-1`；AO Finished+Default 連帶設 CR；Finished **後端嚴驗**必填/Rate-before-Finished/CR comment。
4. **角色控制** `R5`（mutating）：AO 維護 AO 側、CR 維護 CR 側 + comment；**side-scoped 授權**（client 傳的非本側 map 不可信）；**雙層**＝四個 `TB_API_AUTH` seed + service guard 擋非 AO/CR。
5. **存檔/交易** `R6`/`R7`：`CR_SCORE_CARD_COMPLETED` 兩字元語意（本卡 + 另一卡）；單一交易寫評分卡 + summary + checkpoint，任一失敗 rollback；CU 案要更新 `TB_CHECK_POINTS_CU`（非只清 CS）。

**橫切（每階段都適用）**
- **數值防呆** `R8`：欄位精度/長度 BE 必擋、**不得 silent 截斷/四捨五入**；`CUR_RATIO` 是 ×100/÷100 例外（UI 限整數%）。
...
主寫入＝企金評分卡表（PK=案號）；連動＝案件 summary（`CR_SCORE_CARD_COMPLETED` 兩字元）＋ 案件 checkpoint（CS / CU 分支）；讀取＝評分參數表（依申請日取版本）、主借款人表。

## ⚠️ 雷區（依 RD 接手順序）
**動工前必知**
- **分數解析 fail-fast**：參數 `SCORE` 是 `VARCHAR2`，非數字 seed **必須報錯**（`MSG_QUERY_FAIL`），絕不可 silent 當 0。
- **時間 UTC+8**：`scoreDatetime` 用顯式 Asia/Taipei 時鐘，**別吃 JVM/container 預設時區**。
- **雙層授權 + side-scoped**：只設 `TB_API_AUTH` 不夠；service guard 必做，且 AO 只能寫 AO 側、CR 只能寫 CR 側（client 傳的非本側 map 不可信）。

**實作踩坑**
- **數值防呆**：精度/長度 BE 必擋、不得 silent 截斷或四捨五入；`CUR_RATIO` 是 ×100/÷100 例外（UI 限整數%）。
- **calc 禁寫 DB**：計算 endpoint 不得改寫評分卡/summary/checkpoint。
- **Default 連帶效果**：AO Finished + AO Default 會連帶設 CR（`Default`/`-1`/AO 評分日），別漏。
- **參數版本**：用 `MAX(ST_DATE <= 申請日)` 取版本，`END_DATE` 只是 seed metadata、**非 runtime filter**。
- **死碼別重引**：legacy `CR_TRA_LST_MON_CODE*`/`CR_MAX_DUE_CODE*` 是不存在的 DOM id，用 canonical `MAX_OVERDUE_CODE`/`TRANS_12MON_CODE`。
