...
`scoreDatetime` must be formatted as `dd/MM/yyyy HH:mm:ss` using an explicit Asia/Taipei / UTC+8 clock. Owner decision on 2026-06-20 rejects relying on JVM/container default timezone; current `FunctionServiceImpl.getScoreDatetime()` usage of `LocalDateTime.now()` without `ZoneId` is an RD implementation gap until fixed and regression-tested.

`TB_SCORE_CARD_PARAM_DETAIL.SCORE` is physically `VARCHAR2(10)`. BE must convert it with numeric parsing before summing; a nonnumeric `SCORE` seed is a data/query failure and must return `MSG_QUERY_FAIL` with safe diagnostic logging, not silently coerce to zero. Owner decision on 2026-06-20 confirms this fail-fast behavior; current `FunctionServiceImpl.parseScore` behavior that catches `NumberFormatException` and returns `0` is an RD implementation gap until fixed and regression-tested.
...
| backend non-AO roles fall into CR branch if TB_API_AUTH misses | regression/security blocker | SRS requires platform access-denied response (current contract: 401), four endpoint auth seeds, and explicit save service rejection before QA regression or testing deployment. |
...
## NFR
- Security: Save/Finished authorization must be enforced by BE role/context, not only FE disabled state. EPROC00118 `TB_API_AUTH` seeds for select-list/info/calc/save endpoints must exist before QA regression or any testing deployment; missing seeds mean the endpoints are not protected. `epl-save-c0-corporateScorecard` must additionally reject non-AO/non-CR roles inside the save service so a seed/filter miss cannot fall into the CR branch.
