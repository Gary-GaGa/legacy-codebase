...
KHR totals are a display contract with zero fractional digits. Detail persistence may retain the physical `NUMBER(...,2)` scale, but response totals must use the project currency formatter's KHR zero-decimal output and must not expose `.00` or other fractional text.
...
| SEC-002 | R9 | RD/Security | Carry Bible security net: direct calls by roles missing the `TB_API_AUTH.API_ID + REF_FUNCTION_ID + ROLE` tuple must be rejected by BE for both info and save. | `docs/specs/bible/bible-eproposal.md:332`, `docs/specs/bible/bible-eproposal.md:342`; SEC-001 extension 2026-06-24 | spec-frozen: API_ROLE_TUPLE_403_REQUIRED |
