...
- Out of scope: final role-code approval, final CASE_PROGRESS business names, final file-download token design, and final session retirement plan. These remain `@PENDING` evidence items, not SRS approval.
...
Error-code contract: `REDISTRIBUTION_FAILED` is returned as 500 when the redistribution transaction fails; unauthorized API access returns platform authorization error, with `FORBIDDEN_ACTION` reserved for business-role rejection.

### R2 Main task list query **強制點 both**
covers-prd: REQ-002

`epl-list-todolist` with `queryMode=GENERAL` must query by current user, exclude `CASE_PROGRESS` values `03`, `D1`, `R0305`, `R0313`, `R0397`, return borrower display fields, convert sentinel application date `01/01/1900` to blank display, enrich `showRelated` from `TB_RELATED_PARTY_INFO`, and enrich USD/KHR total amount from `TB_LOAN_CONDITION_DETAIL`.
...
Routing must preserve legacy first-page behavior: CA + IS/IU/CS/CU routes to the 0171 family; CAD Maker status `21/22` routes `EPROIS_0910`, Maker `24/25` routes `EPROIS_0920`, CAD Checker `23` routes `EPROIS_0910`, Checker `25` routes `EPROIS_0920`, other CAD routes `MANUALAPP`, and other roles use first-page resolution.
...
Error-code contract: missing `applicationNo` returns `MISSING_APPLICATION_NO`; no reason returns `MISSING_REASON`; `D99` without `otherReason` returns `MISSING_OTHER_REASON`; unauthorized business roles return `FORBIDDEN_ACTION`; transaction/update failure returns `MSG_UPDATE_FAIL`.
...
Error-code contract: missing `applicationNo` returns `MISSING_APPLICATION_NO`; no close reason returns `MISSING_REASON`; `C99` without `otherReason` returns `MISSING_OTHER_REASON`; unauthorized business roles return `FORBIDDEN_ACTION`; transaction/update failure returns `MSG_UPDATE_FAIL`.
...
| DB-D4 `TB_RELATED_PARTY_INFO`, `TB_LOAN_CONDITION_DETAIL`, and `TB_EMP_PROXY` remain active/exact lookup/enrichment tables. | carried | These are read dependencies; `schema.sql` comments/DDL carry the subset needed for QA verification, not ownership of full table design. | `docs/db-diff/02_tables/TB_RELATED_PARTY_INFO.md:11`-`16`, `32`-`43`; `docs/db-diff/02_tables/TB_LOAN_CONDITION_DETAIL.md:11`-`16`, `32`-`47`; `docs/db-diff/02_tables/TB_EMP_PROXY.md:11`-`16`, `32`-`43` |
| DB-D5 `TB_ROLE_DEFINE`, `TB_PROCESS_CODE`, and `TB_API_AUTH` are active/exact but `transpose_method=重新寫入` for role/process/auth configuration. | changed | Treat role names, status names, and API auth seed as configuration baselines requiring seed verification, not legacy-row copy. | `docs/db-diff/02_tables/TB_ROLE_DEFINE.md:11`-`16`, `38`-`40`; `docs/db-diff/02_tables/TB_PROCESS_CODE.md:11`-`16`, `38`-`42`; `docs/db-diff/02_tables/TB_API_AUTH.md:11`-`16`, `38`-`40`; Bible `docs/specs/bible/bible-eproposal.md:390`, `796`-`797` |
...
|---|---|---|---|---|---|
| PENDING-Z001 | @PENDING | PM/SA | Yes for final role approval | PRD TBD-001 `docs/specs/prd/PRD-CDC-EPRO-0001-EPROZ00100-v1.0.md:34`; Bible role dictionary `docs/specs/bible/bible-eproposal.md:796`; FE TLOD rename `docs/refactor-spec/03_artifacts/fe-shared/EPROZ00100/eproz00100-to-do-list.md:96` | Approve role labels and action permissions for `001/002/003/101/102/103/201/202/203/301/302/404/405`, including CAD vs TLOD display wording. |
...
| PENDING-Z009 | @PENDING | RD | No for draft, Yes before auth seed signoff | db-diff `TB_API_AUTH` rewritten `docs/db-diff/02_tables/TB_API_AUTH.md:11`-`16`; SRS endpoints include draft IDs not present in refactor latest map `docs/refactor-spec/02_modules/EPROZ00100.md:15`-`21` | Insert/verify `TB_API_AUTH` rows for all final `epl-*` API IDs and approved roles. |
...
| PRD | SRS | QA |
|---|---|---|
| REQ-001 | R1, R8, R9 | QA-001, QA-002, QA-003, QA-004, QA-005, QA-030, QA-037, QA-038, QA-039 |
| REQ-002 | R2 | QA-006, QA-007, QA-008, QA-009, QA-010 |
| REQ-003 | R3 | QA-011, QA-012, QA-013, QA-014, QA-014A, QA-015 |
| REQ-004 | R4, R8 | QA-016, QA-017, QA-018, QA-019, QA-019A, QA-020, QA-030, QA-037, QA-038, QA-039 |
| REQ-005 | R5, R9 | QA-021, QA-022, QA-022A, QA-023, QA-024, QA-025, QA-026, QA-040, QA-041 |
| REQ-006 | R6, R9 | QA-027, QA-028, QA-029, QA-031, QA-032, QA-032A, QA-033, QA-040, QA-041 |
| REQ-007 | R7, R9 | QA-034, QA-034A, QA-035, QA-036, QA-036A |
