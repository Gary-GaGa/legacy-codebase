...
Given a valid `applicationNo`, `epl-info-c0-financial-evaluation-table-fi` must return page-local initialization data: `applicationName`, this page's `checkpoint` value for `EPROC00120`, and a `financialList` of FI ratio rows.

P-012 only excludes parent progress aggregation from this page DTO. It does not exclude this page's own display header or checkpoint state. Parent all-tab/progress refresh remains delegated to the independent parent/checkpoint endpoint under R19 after save/finish. [ev→R1]
...
Automatic fields are `growthOfGrossLoans` (`GGL`), `cost` (`COST_INCOME`), `roa` (`ROA`), `roe` (`ROE`), and `loans` (`LOANS_DEPOSITS`); R10/R11/R13/R20 deltas apply as to-be rules. [ev→R2]
...
Contract maxlength is DB-driven: `APPLICATION_NO` is `VARCHAR2(30)`, `DATA_SEQ` is `VARCHAR2(2)`, `RATIOS_DATE` is `VARCHAR2(10)`, and stored ratio fields are `VARCHAR2(12)`. FE numeric widget limits are UI constraints and must not override the backend contract. [ev→R3]

### R4 Save draft rows transactionally - 強制點: BE
covers-prd: FR-004

When `isFinish=false`, `epl-save-c0-financial-evaluation-table-fi` must save all submitted rows for one application in a single transaction, replacing existing rows for that `applicationNo`, and mark the page checkpoint as draft/incomplete. Save success uses platform code `COMMON_MSG_SAVE_SUCCESS`. [ev→R4]

### R5 Finish only after BE-required fields pass and update C0 checkpoint - 強制點: BE
covers-prd: FR-005, FR-006

When `isFinish=true`, the backend must enforce required FI fields before mutation is committed, then update `TB_CHECK_POINTS_CS.EPROC00120` or `TB_CHECK_POINTS_CU.EPROC00120` to complete/incomplete using the CS/CU case branch derived from summary attributes. Checkpoint update must affect exactly one row; zero-row or ambiguous update results must throw and roll back the row replacement transaction.

Finished requires each row to include `dataSeq`, `ratiosDate`, and these PRD-required manual fields: `aggregateLargeExposureRatio`, `creditExposureToSingleBeneficiary`, `relatedPartyExposure`, `unhedgedForeignCurrencyRatio`, `liquidityCoverageRatio`, and `solvencyRatio`. Automatic formula fields are system-calculated and are not user-required input fields. `tier1CapitalRatio` remains a manual field but is not promoted to Finished-required without an owner/domain change. [ev→R5]

### R6 Query mode reads saved data without recalculation - 強制點: both
covers-prd: FR-007

When `isQuery=true`, the endpoint must return saved `TB_FINANCIAL_EVALUATION_FI` rows as historical data and must not recalculate from financial statement source tables. That behavior is acceptable for query mode; edit-mode date/change behavior is governed by R13. [ev→R6]

### R7 Carry legacy error codes and rollback behavior - 強制點: BE
covers-prd: FR-001, FR-004, FR-005

The SRS carries PRD error and success keys: `MSG_DATA_NOT_FOUND`, `MSG_OVER_COUNT_LIMIT`, `MSG_QUERY_FAIL`, `COMMON_MSG_ERROR_LON`, `ErrorInputException`, `COMMON_MSG_SAVE_FAIL`, and `COMMON_MSG_SAVE_SUCCESS`.

Platform business errors use HTTP 200 envelopes; authorization failures use `ACCESS_DENIED` / `E405` over HTTP 401. Failed save or checkpoint update must roll back row replacement and checkpoint mutation. A checkpoint update returning zero affected rows is a failed checkpoint update and must throw. [ev→R7]

### R8 Enforce backend authorization and role restrictions on mutation - 強制點: BE
covers-prd: FR-004, FR-005

`epl-save-c0-financial-evaluation-table-fi` is a mutating endpoint. Per owner policy 3, the to-be contract requires two authorization layers before delete/insert/checkpoint mutation: `TB_API_AUTH` seed coverage for the real API path and a service-level guard that verifies case editability, ownership/role, RC old-case `isFinish=true` rejection, and direct-call authorization. `TB_API_AUTH` seed presence alone is not sufficient proof. Before implementation release, RD/DBA must add the actual `TB_API_AUTH` rows for `epl-save-c0-financial-evaluation-table-fi` and service guard logic before any delete/insert/checkpoint update. [ev→R8]

### R9 Reconcile current DTO extras and DB snapshot before release - 強制點: both
covers-prd: FR-003, FR-004

The to-be OpenAPI contract excludes `cetOneCapitalRatio` and `totalCapitalRatio` because PRD and the db-diff snapshot are the higher-priority sources for this SRS. The current DTO/entity fields are recorded as an RD/DBA conformance drift: RD must either remove/ignore them from the page contract or provide a later owner-approved DB/schema delta. They are not Finished-required and must not be treated as EPROC00120 parity evidence in this bundle. [ev→R9]

### R10 Reject source DATA_SEQ/count mismatch - 強制點: BE
covers-prd: FR-002

The to-be behavior is strict mismatch rejection. Balance and income source rows must have the same count and the same ordered `DATA_SEQ` set before ratio computation. Mismatch returns a controlled validation/business error (`ErrorInputException` or the platform equivalent); partial calculation and silent row dropping are forbidden. [ev→R10]

### R11 Render PERIODS=0 annualized ratios as N/A - 強制點: BE
covers-prd: FR-002

When `PERIODS=0`, annualized ROA and ROE display/store as `N/A`. This follows the existing first-period/zero-denominator `N/A` pattern. [ev→R11]

### R12 Keep legacy COST_INCOME formula - 強制點: BE
covers-prd: FR-002

Keep the legacy formula unless a later owner-approved business change says otherwise. `COST_INCOME = OPERATING_EXPENSES * 100 / CONTINUING_OP_INCOME_BT`; denominator zero renders `N/A`. [ev→R12]

### R13 Preserve saved manual values only for matching RATIOS_DATE - 強制點: both
covers-prd: FR-002, FR-003

Edit-mode saved/manual values are preserved only when the saved row matches the same `RATIOS_DATE`. If the source period/date changes or a new source row appears, automatic fields must be recomputed and manual fields must start blank/re-entered for that row; hidden carryover across changed dates is forbidden. [ev→R13]

### R14 Separate Save draft validation from Finished validation - 強制點: BE
covers-prd: FR-004, FR-005

Save draft allows incomplete ratio content but still requires the request envelope (`applicationNo`, `isFinish`, `financialList`) and per-row identity needed for persistence (`dataSeq`, `ratiosDate`). Finished validation is stricter and must enforce the R5 required-field list on the backend, regardless of FE validation. PRD 5.3.1 manual-required fields apply only; automatic formula fields are system-calculated and are not user-required. [ev→R14]

### R15 Preserve RC old-case restriction - 強制點: both
covers-prd: FR-006, FR-008

RC old cases support Save only and must not expose or accept Finished.

The OpenAPI request remains the shared page shape with `isFinish`; it does not carry a client-controlled old-case flag. Backend service guard must derive old-case status from trusted case context and reject direct `isFinish=true` calls before mutation. [ev→R15]

### R16 Use the EPROC00120 function label/key for the migrated page - 強制點: FE
covers-prd: FR-008

The migrated C0 page must use the EPROC00120 function identity/label, including literal key `EPROC00120_FUNC_NAME`. The legacy RC mismatch (`EPROC0_0120_FUNC_NAME` under an 0220 path) is not carried into the new UI and has no backend mutation impact. [ev→R16]

### R17 Keep this SRS C0-only despite shared FI table - 強制點: BE
covers-prd: FR-003, FR-004

`TB_FINANCIAL_EVALUATION_FI` is shared by C0/I0, but this SRS governs only C0 `EPROC00120`. Implementations must scope reads/writes by C0 application/page context and must not migrate, delete, or reinterpret I0 data under this bundle. [ev→R17]
...
|---|---|---|---|
| R1 | Current response DTO exposes only `financialList`, so RD must add `applicationName` and `checkpoint` to close the conformance gap. Legacy 0120 does `auditLog`, `getOther`, and `query`. | RD adds `applicationName`/`checkpoint` to info DTO; parent refresh delegated to independent endpoint under R19. | PRD query flow at `docs/specs/prd/PRD-CDC-EPRO-0001-EPROC00120-v1.0.md:198-203`, response fields `dataMap.APPLICATION_NAME`, `dataMap.check`, `fiList` at `:449-451`; legacy `[CODE:legacy-epro/JavaSource/com/cathaybk/epro/c0/trx/EPROC0_0120.java:47@2ae96d0]`, `[CODE:legacy-epro/JavaSource/com/cathaybk/epro/c0/trx/EPROC0_0120.java:57@2ae96d0]`; current DTO `[CODE:backend/src/main/java/khd/svc/epro/dto/response/corporate/csuFinancialEvaluationTableFi/InfoCsuFinancialEvaluationTableFiResponse.java:12@2ae96d0]`. |
| R2 | Current backend aligns income by `dataSeq` during calculation and computes the automatic fields; this is the code-as-baseline. | code-as-baseline with R10/R11/R13/R20 deltas applied as to-be rules. | PRD formulas at `docs/specs/prd/PRD-CDC-EPRO-0001-EPROC00120-v1.0.md:548-554`; current align `[CODE:backend/src/main/java/khd/svc/epro/service/corporate/impl/CsuFinancialEvaluationTableFiServiceImpl.java:116@2ae96d0]`, compute `[CODE:backend/src/main/java/khd/svc/epro/service/corporate/impl/CsuFinancialEvaluationTableFiServiceImpl.java:152@2ae96d0]`. |
| R3 | Current backend DTO uses `@Size(max=12)` for ratio fields. | Contract maxlength is DB-driven; FE numeric widget limits are UI constraints and must not override backend contract. | PRD field mapping at `docs/specs/prd/PRD-CDC-EPRO-0001-EPROC00120-v1.0.md:489-509`; DB snapshot `docs/db-diff/02_tables/TB_FINANCIAL_EVALUATION_FI.md:38-56`; current DTO `[CODE:backend/src/main/java/khd/svc/epro/dto/request/corporate/csuFinancialEvaluationTableFi/SaveCsuFinancialEvaluationTableFiRequest.java:55@2ae96d0]` and `[CODE:backend/src/main/java/khd/svc/epro/dto/request/corporate/csuFinancialEvaluationTableFi/SaveCsuFinancialEvaluationTableFiRequest.java:111@2ae96d0]`. |
| R4 | Current backend save is transactional, deletes existing rows, inserts all request rows, updates checkpoint, and returns an empty map. Legacy 0120 save deletes by application and inserts rows in a transaction. | Full replacement + `check=Y` + success response; save success uses platform code `COMMON_MSG_SAVE_SUCCESS`. | PRD Save behavior at `docs/specs/prd/PRD-CDC-EPRO-0001-EPROC00120-v1.0.md:293-299`, success mapping at `:466-475`; legacy `[CODE:legacy-epro/JavaSource/com/cathaybk/epro/c0/module/EPROC0_0120_mod.java:196@2ae96d0]` and `[CODE:legacy-epro/JavaSource/com/cathaybk/epro/c0/module/EPROC0_0120_mod.java:224@2ae96d0]`; current `[CODE:backend/src/main/java/khd/svc/epro/service/corporate/impl/CsuFinancialEvaluationTableFiServiceImpl.java:195@2ae96d0]`, `[CODE:backend/src/main/java/khd/svc/epro/service/corporate/impl/CsuFinancialEvaluationTableFiServiceImpl.java:214@2ae96d0]`, `[CODE:backend/src/main/java/khd/svc/epro/service/corporate/impl/CsuFinancialEvaluationTableFiServiceImpl.java:215@2ae96d0]`. |
| R5 | Current validator only checks `dataSeq`; that is an RD conformance gap, not an open SRS decision. | Owner correction 2026-06-24 confirms Finished-required set follows PRD 5.3.1: automatic formula fields are system-calculated and not user-required; `tier1CapitalRatio` not promoted without owner/domain change. Decision P-005. | PRD checkpoint/Finished at `docs/specs/prd/PRD-CDC-EPRO-0001-EPROC00120-v1.0.md:353-359`, required manual fields at `:267-270`; current validator `[CODE:backend/src/main/java/khd/svc/epro/dto/request/corporate/csuFinancialEvaluationTableFi/FinancialEvaluationWhenFinishValidator.java:33@2ae96d0]`. |
| R6 | Current backend returns saved evaluation rows whenever rows exist before edit/query branching. Legacy query mode reads saved table rows first. | Query mode returns saved rows without recalculation; edit-mode date/change behavior governed by R13. | PRD historical query at `docs/specs/prd/PRD-CDC-EPRO-0001-EPROC00120-v1.0.md:389-408`; legacy `[CODE:legacy-epro/JavaSource/com/cathaybk/epro/c0/module/EPROC0_0120_mod.java:67@2ae96d0]` and `[CODE:legacy-epro/JavaSource/com/cathaybk/epro/c0/module/EPROC0_0120_mod.java:88@2ae96d0]`; current `[CODE:backend/src/main/java/khd/svc/epro/service/corporate/impl/CsuFinancialEvaluationTableFiServiceImpl.java:85@2ae96d0]` and `[CODE:backend/src/main/java/khd/svc/epro/service/corporate/impl/CsuFinancialEvaluationTableFiServiceImpl.java:112@2ae96d0]`. |
| R7 | Current `updateCheckpoint()` does not prove affected-row enforcement, so RD must close that conformance gap before release. Current save is `@Transactional`. | Platform business errors use HTTP 200 envelopes; authz failures use `ACCESS_DENIED`/`E405` over HTTP 401; failed save/checkpoint rolls back; zero affected rows must throw. | PRD error mapping at `docs/specs/prd/PRD-CDC-EPRO-0001-EPROC00120-v1.0.md:466-475`, `ErrorInputException` at `:572`, rollback principle at `:576-583`; current `[CODE:backend/src/main/java/khd/svc/epro/service/corporate/impl/CsuFinancialEvaluationTableFiServiceImpl.java:195@2ae96d0]`. |
| R8 | c0 auth findings show missing EPROC00120 auth rows; current save service performs mutation without source evidence of a service-level edit-role/old-case rejection. Those are downstream RD/DBA conformance gaps. | Per owner policy 3, two-layer authorization requirement: `TB_API_AUTH` seed + service-level guard; seed presence alone insufficient. Decision P-011. | c0 findings `docs/build-tasks/c0-authz-sql-findings.md:71-72`; current `[CODE:backend/src/main/java/khd/svc/epro/service/corporate/impl/CsuFinancialEvaluationTableFiServiceImpl.java:202@2ae96d0]` and `[CODE:backend/src/main/java/khd/svc/epro/service/corporate/impl/CsuFinancialEvaluationTableFiServiceImpl.java:255@2ae96d0]`. |
| R9 | Current backend request/response DTOs also expose `cetOneCapitalRatio` and `totalCapitalRatio`; current entity maps them to `CET_ONE_CAPITAL` and `TOTAL_CAPITAL`. Recorded as RD/DBA conformance drift. | PRD/db-diff are higher-priority sources; exclude the two fields from to-be OpenAPI; RD removes/ignores or provides later owner-approved DB/schema delta. Decision P-010. | PRD/db-diff at `docs/specs/prd/PRD-CDC-EPRO-0001-EPROC00120-v1.0.md:489-509` and `docs/db-diff/02_tables/TB_FINANCIAL_EVALUATION_FI.md:38-56`; current DTO `[CODE:backend/src/main/java/khd/svc/epro/dto/request/corporate/csuFinancialEvaluationTableFi/SaveCsuFinancialEvaluationTableFiRequest.java:102@2ae96d0]`; entity `[CODE:backend/src/main/java/khd/svc/epro/entity/TBFinancialEvaluationFiEntity.java:61@2ae96d0]`. |
| R10 | Source count/key mismatch could otherwise lead to partial or unhandled calculation. | Owner decision P-001 closes PRD TBD-001: strict mismatch rejection; partial calculation and silent row dropping forbidden. REF-D1. | Owner 2026-06-24; PRD negative case `docs/specs/prd/PRD-CDC-EPRO-0001-EPROC00120-v1.0.md:614`. |
| R11 | Current backend helper returns blank for zero average/period cases; that is an RD conformance gap. | Owner decision P-002 closes PRD TBD-002: `PERIODS=0` annualized ROA/ROE render `N/A`. REF-D2. | Owner 2026-06-24; current helper `[CODE:backend/src/main/java/khd/svc/epro/service/corporate/impl/CsuFinancialEvaluationTableFiServiceImpl.java:344@2ae96d0]` and `[CODE:backend/src/main/java/khd/svc/epro/service/corporate/impl/CsuFinancialEvaluationTableFiServiceImpl.java:357@2ae96d0]`. |
| R12 | — | Owner decision P-003 closes PRD TBD-003: keep legacy `COST_INCOME` formula unless a later owner-approved business change says otherwise. | Owner 2026-06-24; legacy 0120 formula `[CODE:legacy-epro/JavaSource/com/cathaybk/epro/c0/module/EPROC0_0120_mod.java:113@2ae96d0]` and `[CODE:legacy-epro/JavaSource/com/cathaybk/epro/c0/module/EPROC0_0120_mod.java:119@2ae96d0]`. |
| R13 | Legacy preserves manual fields by ratio-date match. Current backend returns saved rows before recomputation whenever saved rows exist; RD must align. | Owner decision P-004 closes PRD TBD-004: preserve manual values only for matching `RATIOS_DATE`; date changes require re-entry; hidden carryover forbidden. REF-D3. | Owner 2026-06-24; legacy `[CODE:legacy-epro/JavaSource/com/cathaybk/epro/c0/module/EPROC0_0120_mod.java:97@2ae96d0]` and `[CODE:legacy-epro/JavaSource/com/cathaybk/epro/c0/module/EPROC0_0120_mod.java:113@2ae96d0]`; current `[CODE:backend/src/main/java/khd/svc/epro/service/corporate/impl/CsuFinancialEvaluationTableFiServiceImpl.java:85@2ae96d0]` and `[CODE:backend/src/main/java/khd/svc/epro/service/corporate/impl/CsuFinancialEvaluationTableFiServiceImpl.java:112@2ae96d0]`. |
| R14 | — | Owner decision P-005 closes PRD TBD-005: Save draft minimal persistence validation, Finished BE enforces R5 list regardless of FE; owner correction 2026-06-24 adopts PRD 5.3.1 manual-required fields only. | Owner 2026-06-24. |
| R15 | Legacy RC JSP shows Finished only when `!attrMap.isOld`; JS updates parent done style only when not old. Current backend has no source evidence of the old-case guard. | Owner decision P-006 closes PRD TBD-006: RC old cases Save-only, no Finished; backend derives old-case from trusted context and rejects direct `isFinish=true`. | Owner 2026-06-24; legacy `[CODE:legacy-epro/WebContent/html/cathaybk/system/epro/c0/EPROC0_0200/EPROC00220.jsp:165@2ae96d0]`, `[CODE:legacy-epro/WebContent/html/cathaybk/system/epro/c0/EPROC0_0200/EPROC00220.jsp:170@2ae96d0]`, `[CODE:legacy-epro/WebContent/html/cathaybk/system/epro/c0/EPROC0_0200/EPROC00220_JS.jsp:122@2ae96d0]`, `[CODE:legacy-epro/WebContent/html/cathaybk/system/epro/c0/EPROC0_0200/EPROC00220_JS.jsp:125@2ae96d0]`. |
| R16 | Legacy RC mismatch uses `EPROC0_0120_FUNC_NAME` under an 0220 path. | Owner decision P-007 closes PRD TBD-007: migrated C0 page uses EPROC00120 identity/label incl. literal `EPROC00120_FUNC_NAME`; legacy mismatch not carried, no backend mutation impact. | Owner 2026-06-24. |
| R17 | `TB_FINANCIAL_EVALUATION_FI` is shared by C0/I0. | Owner decision P-008 closes PRD TBD-008: this SRS governs only C0 `EPROC00120`; scope reads/writes by C0 context; do not migrate/delete/reinterpret I0 data. | Owner 2026-06-24. |
| R18 | Formal corporate refactor baseline for `EPROC00120` is UNFOUND. | Owner decision P-009 closes the missing-baseline gap under policy 1: use legacy `EPROC0_0120`/`EPROC0_0220` + current implementation at `2ae96d0` as as-is baseline, deltas in R10-R20; no formal artifact fabricated. | Owner policy 1; commit `2ae96d0`. |
| R19 | PRD/legacy parent-progress fields exceed current DTO/service evidence. Cross-page triggers (0119/0219 save resets 0120/0220 checkpoint; 0110/0210 Business Type change deletes FI evaluation/statement data) remain traceable obligations owned by parent/source pages. | Owner decision P-012 closes only the parent-progress DTO gap: save does not add `pageCheckMap`/`isAllTabsCheck`/aggregation; R1 page-local `applicationName`/`checkpoint` retained; parent aggregation via independent endpoint. | Owner 2026-06-24; PRD parent progress `docs/specs/prd/PRD-CDC-EPRO-0001-EPROC00120-v1.0.md:453-464`, cross-page tests `:619-621`. |
| R20 | Legacy 0120 uses `ROUND_HALF_DOWN` plus `formatTwo` in formula branches. Current backend uses `RoundingMode.HALF_UP` helper paths; that is an RD conformance bug under owner policy 2. | Owner decision P-013 closes the rounding decision: automatic ratios use legacy `HALF_DOWN`; intermediate scale ≥10, two-decimal display/storage, preserve `N/A`. | Owner 2026-06-24; legacy `[CODE:legacy-epro/JavaSource/com/cathaybk/epro/c0/module/EPROC0_0120_mod.java:117@2ae96d0]`, `[CODE:legacy-epro/JavaSource/com/cathaybk/epro/c0/module/EPROC0_0120_mod.java:124@2ae96d0]`, `[CODE:legacy-epro/JavaSource/com/cathaybk/epro/c0/module/EPROC0_0120_mod.java:138@2ae96d0]`, `[CODE:legacy-epro/JavaSource/com/cathaybk/epro/c0/module/EPROC0_0120_mod.java:148@2ae96d0]`, `[CODE:legacy-epro/JavaSource/com/cathaybk/epro/c0/module/EPROC0_0120_mod.java:158@2ae96d0]`, `[CODE:legacy-epro/JavaSource/com/cathaybk/epro/c0/module/EPROC0_0120_mod.java:164@2ae96d0]`; current `[CODE:backend/src/main/java/khd/svc/epro/service/corporate/impl/CsuFinancialEvaluationTableFiServiceImpl.java:329@2ae96d0]`, `[CODE:backend/src/main/java/khd/svc/epro/service/corporate/impl/CsuFinancialEvaluationTableFiServiceImpl.java:339@2ae96d0]`, `[CODE:backend/src/main/java/khd/svc/epro/service/corporate/impl/CsuFinancialEvaluationTableFiServiceImpl.java:352@2ae96d0]`, `[CODE:backend/src/main/java/khd/svc/epro/service/corporate/impl/CsuFinancialEvaluationTableFiServiceImpl.java:355@2ae96d0]`. |
