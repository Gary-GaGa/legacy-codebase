...
>
> **兩種來源**：① **PRD**（PM 的 PRD 為 what，用 `.claude/skills/prd-to-srs` 轉 → 規則追溯 PRD REQ-id、TBD 寫 `@PENDING`、頁已存在則標 as-is/to-be）——**主路**；② **鏡像 i0**（無 PRD 時，既有新碼為 oracle，引 i0 `file:line`）。
...
## 怎麼用
- **有 PRD（主路）**：叫 `/prd-to-srs`（帶 PRD）自動產 bundle（見 `EPROZ00800/`）——規則追溯 PRD REQ-id、TBD→`@PENDING`、endpoint 寫真實 `epl-*`、頁已存在則標 as-is/to-be。
- **無 PRD（鏡像 i0）**：`cp -r EPROZ00800 <新FUNC_ID>` 換 ID/endpoint/表名，再：
  1. **openapi.yaml**：對既有 i0 controller 的 DTO 填**確切**欄位 + `@JsonProperty`（前端契約不可變）。
  2. **schema.sql**：列**本頁真的會讀寫**的表 + 欄位（JIT 從 `db-schema-catalog.md` 抽，不要全表貼）。
