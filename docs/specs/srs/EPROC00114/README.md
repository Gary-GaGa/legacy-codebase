...
4. **Rate 評分** `R4`：save 流程內**後端**依 seed 算總分、查**抵押**風險等級（用對的風險變數欄，非個金那組，確切欄見 spec `R4`）、推日期；保留小數精度；異常/查無 seed/溢位 = Rate fail。
...
## NFR（精確見 spec `## NFR`）
授權（雙層、seed 單獨不足）／交易（評分表+完成狀態+checkpoint 同生同滅、Rate fail 不覆蓋已存值）／**精度**（分數對齊 DB 欄精度（見 schema）、非數/溢位 fail-fast）／**稽核**（查/Rate/存/Finish 留 masked audit、不印完整借款人身分/評語）／**觀測**（交易帶 correlation id）／量過大走 over-count 錯。
...
- **風險變數別用錯**：抵押風險等級查抵押風險等級欄，**不是**個金那組個人風險等級欄（現碼用錯＝RD 要修；確切欄名見 spec `R4`/schema）。
- **雙層授權**：只設 API 授權不夠，service guard 必做；AO 不可寫 CR comment（PENDING-009/009A）。

**實作踩坑**
- **精度**：分數用 BigDecimal/fail-fast 對齊 DB 欄精度（見 schema），別整數截斷、別 null→0 吞錯（PENDING-010）。
- **無獨立 calc endpoint**：別重建不存在的計算端點；Rate 掛 save path、結果走 `rateResult`。
- **CS-only**：別因 schema 有 CU 欄就寫 CU；非 CS 直呼 save/finish 要擋、不得 mutate。
- **位址欄名**：用抵押品位址欄，別用 stale 的舊位址欄（確切欄名見 spec `R6`/schema）。
