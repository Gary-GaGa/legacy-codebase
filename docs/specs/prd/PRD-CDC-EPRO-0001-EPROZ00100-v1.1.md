...
> **快照說明**：本檔＝owner 於 2026-06-18 提供之 `CDC-EPRO-0001 EPROZ00100` PRD（內文 Version 1.1）原樣 ingest，作為 `docs/specs/srs/EPROZ00100/` 的上游 SSOT（gateⒷ `covers-prd` / gateⒺ 錯誤碼承載對此快照驗）。**內容權威＝PM 版 PRD**；本檔僅快照，勿在此裁定 TBD。
...
## 6.1 Endpoint（理想化 REST，SRS 轉真實 epl-* RPC）
...
## 8. 測試案例草案（Given/When/Then；SRS qa-cases 轉 test-ready）
