...
- **Bible↔PRD 對照表（trace sidecar）**：`trace-<docId>-<funcId>.md`（例：`trace-CDC-EPRO-0001-EPROZ00800.md`）——快照不可編，追溯標記放 sidecar。表 A（Bible BR→PRD；無 REQ 對應**必標** `BP-n`/`@PENDING`）+ 表 B（PRD REQ→Bible 錨點；快照每個 REQ 都要入表）。**`check-srs-bundle.py` gate⑥ 機械讀**：`covers-prd` 懸空=FAIL、trace 缺漏=advisory warn。外部 PRD 改版重新快照時 trace 同步重對。
