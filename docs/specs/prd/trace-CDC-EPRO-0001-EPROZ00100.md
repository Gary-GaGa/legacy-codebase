...
| 〔非 BR〕報表查詢入口（Bible:261，`EPROZ00600`）| REQ-007 Proposal Download | PARTIAL → `Z006` 已裁（owner 06-19）→ 殘 RD/Security 實作 | 下載 token/path 策略：owner 06-19 裁一次性短時效 `downloadToken`、殘 RD/Security 實作（spec.md:162）；Bible 僅旅程級報表錨點 |
...
| REQ-003（CAD Task Search）| 角色 402 CAD（Bible:796，定義另見 Bible:406）＋案件類型 `LON_TYPE_CODE` 01-04（Bible:792）| **承載**（R3, spec.md:79-84）：active proxy＝as-is carried（legacy 自 `TB_EMP_PROXY` 加入 `IN_CURRENT_USER_ID`、非 regression）；draft 約束 docNo↔`NO` mapping＝`Z004` 已裁（owner 06-19）→ 殘 RD 實作、date-range 六月限制＝`Z005` 已裁（owner 06-19）→ 殘 BE guard 實作 |
...
| REQ-007（Proposal Download）| **Bible 無明確錨點**（最近＝報表查詢 Bible:261）| code-derived；下載 token/path 策略 → `Z006` 已裁（owner 06-19）→ 殘 RD/Security 實作（spec.md:162） |
