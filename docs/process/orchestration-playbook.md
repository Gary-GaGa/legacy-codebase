...
讀 per-page-reinventory-matrix「PRD→SRS 佇列 + ledger」表
 ...
 → while 佇列仍有 status=prd-ready 列（可多頁同時 ready）：           ← drain 迴圈
     取 risk-tier 最前、status=prd-ready 的【一頁】（序列、一次只一頁；同 risk 依表序）
      ...
      → SRS N 軸驗證（§4b 的 A–F 七正交、跨模型）→ 採納修正後再審一輪
...
讀 per-page-reinventory-matrix ledger（同一張，狀態欄擴 rd-ready/rd-done/…）
 ...
 → while 佇列仍有 rd-ready（序列、一次一頁、不並行）：
     取 risk-tier 最前一頁
      ...
      → 達標 → ledger status=rd-done、填 code 路徑（commit/PR）→ 進 DoD 閘門牆（gate④ 跑 qa-cases、⑤覆蓋率 + 人審）→ owner 蓋 done
...
4. SRS N 軸驗證：依 playbook §4b 各 spawn 一 read-only sub-agent 跑 A–F 軸（各獨立 session、不同指示、最好跨模型；非單一 spec-reviewer；低風險頁可 A+E+G、risk-tier T1 全 A–F）→ 採納修正後再審一輪。
