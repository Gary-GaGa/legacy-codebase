...
> 〔QA flow 驗證軸（原 §4d）已移除——QA Agent Flow 暫不納入；先把 Bible→PRD→SRS→RD→DoD 跑順。**QA 產生/驗收 亦一併暫拔除**：bundle 不再產 qa-cases、gate④/⑤（qa-cases/覆蓋率）暫停（`docs/specs/qa-to-test.md` 休眠）；code 階段驗證暫由機械閘門＋`spec-reviewer`＋Phase V runtime 自驗接。日後再決定 QA flow 何處介入。〕
...
讀 per-page-reinventory-matrix ledger（同一張，狀態欄擴 rd-ready/rd-done/…）
 ...
 → while 佇列仍有 rd-ready（序列、一次一頁、不並行）：
     取 risk-tier 最前一頁
      ...
      → 達標 → ledger status=rd-done、填 code 路徑（commit/PR）→ 進 DoD 閘門牆（①②③⑥⑦LLM審 + 人審；④⑤ QA 暫拔除）→ owner 蓋 done
