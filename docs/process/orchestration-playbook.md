...
## 1b. B/C 類決策卡範本（agent 推 owner 裁前必備）
B/C 類待決推 owner 時，agent **不只「列出」**，須整理成可一頁裁定的**決策卡**（逐項）：
- **ID + 源**：`@PENDING`/TBD/RP/P id + 源自哪個 PRD 缺陷/REQ。
- **問題**：一句話講要裁什麼。
- **證據**：已蒐集的（DB 可查？code 可判？設計依據？）+ `file:line`。
- **選項**：2–3 個可行解 + 各自影響/風險。
- **建議**：agent 推薦哪個 + 為什麼（**標「建議、非決策」**）。
- **誰裁**：owner / PM / domain。
> owner 可逐項裁、或**批次採建議**（＝owner 明示接受該建議集，仍是 owner 決策、非 agent 自裁）。活例＝`build-tasks/srs-finalize-c0-115-119-120.md §owner 決策`。
...
> SRS 是「**還沒 code**」的文件——§4 三軸（git diff／FE↔BE DTO／build／runtime）對一份 `spec.md` 幾乎無話可說。SRS 階段的「多角度」改用下列 **六正交軸（A–F；原 G 可測試性隨 QA 暫拔除）**，各 read-only、獨立 session、**最好跨模型**（同 §4 反 correlated-blindness）。**一份 SRS 完成＝這 N 軸全無 Blocker**＝owner「每份 SRS 完成後多開 sub-agent 不同角度再驗」的落地（取代「單一 spec-reviewer」）。
| 軸 | 只看 | 對應易疏漏陷阱 |
|---|---|---|
...
| ~~**G. 可測試性**~~ | 〔**暫拔除**：隨 QA 產生/驗收暫停；qa-cases 不存在時本軸不適用。恢復 QA 後重啟〕 | — |
...
### gate ⑧ runtime conformance（per-page DoD；有 harness manifest 即 blocking）
> **第四種驗證＝runtime conformance**：跑活 endpoint 對真 DB，oracle＝**openapi 契約＋等價唯讀 SQL（DB 當判官）**，驗「API 忠實反映 DB＋守自己 openapi 契約」（RV-1/RV-2、langType、必填欄）。**非 QA**（QA＝per-Rn authored acceptance、已暫拔；二者正交）。補**靜態閘 ①②③⑥ 的 runtime 盲區**——①只比對 DTO 宣告，抓不到「空分支沒填必填欄」這種**只有跑起來才現形**的行為（RI-2 即此）。
- **執行**：`tools/phase-v-run.ps1`（local-env up→serviceability smoke→per-role JWT→harness→finally down；經已驗收 env manager `process/local-env-manager.md`）。
- **範圍＝讀型 only（owner 2026-06-27）**：gate ⑧ 在 RD Flow **只驗讀型 API↔DB conformance**（v1 langType/RI、v2 c0 讀型 sele/info/list）。**寫入 save/submit（v3）不在 gate ⑧**——案件流程耦合（狀態/角色/上游/checkpoint）、單頁難孤立 → 歸**未來 QA Flow 整合 tier**（見 `decisions.md`「gate ⑧ 範圍＝讀型」列）。過渡：高風險寫入 fixture-case 半自動 + 廣面 UAT。
- **blocking 範圍（漸進）**：**有 harness manifest 的頁＝rd-done 前 blocking**；無 manifest 的頁維持 ①②③⑥+RD 軸（不阻、標待補 harness）。manifest 隨覆蓋漸增、可隨 RD 從 SRS 抽讀型 API↔DB 一致斷言產出。
- **失敗三分類 × 自修界線**（用 runner sentinel 歸因，決定 RD 能不能自修）：
  | 分類 | sentinel | RD 處置 |
  |---|---|---|
  | infra | `ENV_NOT_READY` | env/harness 問題、非產品碼 bug → 修 env/harness、**不得改產品碼湊綠** |
  | auth | `AUTH_FAILED` | role/授權 → per-case role 修 manifest；涉 `TB_API_AUTH` DML/service-guard → escalate（人審） |
  | assertion | 200 但內容/契約不符 | ✅ RD **自修**（碼對齊已核准契約，如 RI-2 選項移出空分支）；**揭露契約模糊/判斷/需改 openapi/安全 → 🛑 escalate 不自裁**（C 類/@PENDING/owner，像 RI-2 先確認 openapi 權威才修） |
- **鐵則**：只有 **assertion-conformance 可自修**；**不靠放寬契約/抑制斷言「湊綠」**（守判斷題不自裁、斷言失敗只報）。⑧≠QA（不算復活 QA）。

> 〔QA flow 驗證軸（原 §4d）已移除——QA Agent Flow 暫不納入；先把 Bible→PRD→SRS→RD→DoD 跑順。**QA 產生/驗收 亦一併暫拔除**：bundle 不再產 qa-cases、gate④/⑤（qa-cases/覆蓋率）暫停（`docs/specs/qa-to-test.md` 休眠）；code 階段 runtime 驗證改由**上述 gate ⑧ runtime conformance**（＋機械閘門＋`spec-reviewer`）承接——**⑧ 是 conformance、非 QA acceptance**。日後再決定 QA flow 何處介入。〕
...
讀 per-page-reinventory-matrix「PRD→SRS 佇列 + ledger」表
 ...
 → while 佇列仍有 status=prd-ready 列（可多頁同時 ready）：           ← drain 迴圈
     取 risk-tier 最前、status=prd-ready **且 prd 實檔存在於 docs/specs/prd/** 的【一頁】（序列、一次只一頁；同 risk 依表序）
      → 〔prd-ready 但 PRD 實檔不存在（placeholder 路徑、owner 尚未放實檔）＝**跳過不 pick**，留 prd-ready，批末回報「待放實檔」；**不臆造 PRD、不空產 SRS**〕
      → spawn 獨立 sub-agent 跑 prd-to-srs（dispatch 卡 prompt，填 funcId/PRD 路徑）→ 產 bundle 到母資料夾 docs/specs/srs/<funcId>/（Model A：local、gitignored）
      → 機械 gate：check-srs-bundle.py exit 0（含 gateⓇ reconcile）
      → SRS N 軸驗證（§4b 的 A–F 六正交、跨模型）→ 採納修正後再審一輪
 ...
 → 佇列已無「prd-ready ∧ 實檔存在」的可 pick 頁（皆 in-review/blocked，或 prd-ready-但-無實檔）→ 停【batch checkpoint】，彙總「整批」等人審/裁 TBD（含「prd-ready 待放實檔」清單）
   〔終止保證：每輪取一頁，結局必為 in-review 或 blocked，兩者都離開「prd-ready ∧ 實檔存在」集合 → |prd-ready ∧ 實檔存在| 每輪嚴格遞減 → 最多 N 輪必停；無實檔的 prd-ready 頁不被 pick、不影響遞減〕
...
- **首批放量上限（漸進）**：首次批量**建議 ≤5 頁、同 risk-tier**（低風險先；T1 ≤3）；證實人審/N 軸跟得上再擴大批量。**禁止為衝吞吐而降 N 軸**（**每頁一律全 A–F、多 sub-agent、跨模型，§4b；drain 對齊 pilot、不降軸**）。
- **Context Window（上下限）**：批量 × 全 A–F 多 sub-agent 靠**隔離+壓縮+分批**控在 context 內——主控只存 ledger+當頁 PASS/Blocker 路徑（不吞 bundle/transcript 全文）、**每頁完即丟細節**（跨頁不累積）；sub-agent 各自獨立 session **自讀原檔**（不吃摘要＝防空審）；單批頁數設上限、接近 context 上限即停輸出 ledger、靠 resume 冪等下批接續。細節＝`prd-to-srs-orchestrator-drain.md §1b/§2c`。
...
- **owner 控放量、orchestrator 控 drain**：佇列無期限、risk-tier 前段先；**owner 放多少 PRD（放實檔的頁才會被 drain）就 drain 多少**——「不急著轉完」指 owner 放 PRD 的步調，非 drain 本身。owner 未放 PRD 的頁＝not-started、跳過不臆造。
- **🚧 placeholder prd-ready 守門（實檔存在才 pick）**：ledger 可能把頁批次標 `prd-ready` 但 prd 欄填 **placeholder 命名（實檔尚未落母資料夾）**（見 matrix「placeholder PRD caveat」）。**pick 條件＝`status=prd-ready` ∧ 該 prd 路徑實檔存在於 `docs/specs/prd/`**；**prd-ready 但 PRD 實檔不存在＝跳過不 pick**（留 prd-ready、批末列「待放實檔」），**絕不臆造/空產 SRS**。＝「`prd-ready` 是 owner greenlit 排程、實檔落地才是真正放行 drain」。owner 逐批放實檔＝真正的放量節流（標 prd-ready 只是排隊）。
...
讀 per-page-reinventory-matrix ledger（同一張，狀態欄擴 rd-ready/rd-done/…）
 ...
 → while 佇列仍有 rd-ready（序列、一次一頁、不並行）：
     取 risk-tier 最前一頁
      ...
      → ⑧ runtime conformance（§4c：有 harness manifest 的頁 blocking，跑 phase-v-run.ps1；三分類歸因、assertion-conformance 自修、契約模糊/infra/auth escalate；無 manifest 頁略過待補）
      → RD 軸（§4c：contract/scope/regression/強制點落實，跨模型、advisory）→ 採納修正後再審一輪
      → 達標（①②③⑥+⑧〔有 harness 者〕綠 + RD 軸無 Blocker）→ ledger status=rd-done、填 code 路徑（commit/PR）→ 進 DoD 閘門牆（①②③⑥⑦LLM審〔+⑧ runtime conformance〕 + 人審；④⑤ QA 暫拔除）→ owner 蓋 done
      → 未達標（build 紅 / verify-c0 違規 / 軸有 Blocker 需 C 類）→ status=blocked+原因（離開 rd-ready）→ 續下一頁
 → 佇列無 rd-ready → batch checkpoint
```
- 守則全 mirror §5b（序列非並行、context 衛生、circuit-breaker、單頁 FAIL 標 blocked、首批漸進）。**終點＝`rd-done`、不得自宣 Done**（待 DoD 閘門牆 ①②③⑥⑦⑧ + 人審 + owner 蓋章）。
- **T1（金錢/評分/授權）頁＝per-page 多 agent 驗證點**（owner 改自「per-page 人審停點」）：code 產完即 spawn RD 軸（§4c）≥4 隻跨模型 read-only 驗 → Blocker 則**修該頁再驗** → 無 Blocker 即 rd-done、**drain 不停人、續下一頁**（不逐頁停人審）。**correlated-blindness 防護由 per-page 多 agent 跨模型驗承載**（非人審停點＝原規則的實質保留、只換交付者）；**人審收斂到批末 + owner Done**。批末另跑**批次多 agent 驗證**（跨頁一致性 + 跨批 regression + RD 軸對合併 diff）→ 再交人審/owner。最高風險頁仍可 owner 指定單跑。

> 〔QA 軌迴圈（原 §5d）已移除——QA Agent Flow 暫不納入。RD 後直接進 DoD 閘門牆（gate①②③⑥⑦；**gate④ qa-cases／⑤覆蓋率 隨 QA 產生/驗收暫拔除而暫停**），owner 蓋 `done`。待 Bible→PRD→SRS→RD→DoD 跑順後，再決定 QA flow（三層測試/報告）何處介入。〕
...
2. spawn 獨立 sub-agent 跑 prd-to-srs（用 build-tasks/prd-to-srs-codex-dispatch.md 的 prompt，填該 funcId/PRD 路徑）→ 產 bundle 到母資料夾 docs/specs/srs/<funcId>/（Model A：local、gitignored）。
3. 機械 gate：python scripts/check-srs-bundle.py docs/specs/srs/<funcId> 必 exit 0（含 gateⓇ reconcile）。
4. SRS N 軸驗證：依 playbook §4b 各 spawn 一 read-only sub-agent 跑 A–F 軸（各獨立 session、不同指示、最好跨模型；非單一 spec-reviewer；**T1 與低風險頁一律全 A–F、不降軸**）→ 採納修正後再審一輪。
...
- **SRS 軌 N 軸（§4b）**：Codex orchestrator 依 §4b briefs 各 spawn 一 read-only sub-agent（跨模型）跑 A–F 軸；**不需 per-axis toml**（briefs 權威＝§4b，同薄殼指標）；axis A＝既有 `spec-reviewer.toml`。
...
> **SRS 軌 drain 模式已啟用**：pilot 已過（`EPROZ00100`/`EPROC00118` 重產 → Approved）→ SRS 軌改 **drain 迴圈**（§5b/§6b）：允許多頁同時 `prd-ready`、**序列一次一頁**逐頁過 gate＋N 軸，**checkpoint 從「每頁停」改為「整批 prd-ready 清空後停一次」**（batch checkpoint）。守則不變：序列非並行、每頁全 gate＋N 軸、終點 `in-review`（不自升 approved）、C 類不碰、單頁 FAIL 標因續跑。
