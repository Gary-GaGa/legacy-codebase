...
## 1b. B/C 類決策卡範本（agent 推 owner 裁前必備；2026-06-24 立）
...
> SRS 是「**還沒 code**」的文件——§4 三軸（git diff／FE↔BE DTO／build／runtime）對一份 `spec.md` 幾乎無話可說。SRS 階段的「多角度」改用下列 **六正交軸（A–F；原 G 可測試性隨 QA 2026-06-24 暫拔除）**，各 read-only、獨立 session、**最好跨模型**（同 §4 反 correlated-blindness）。**一份 SRS 完成＝這 N 軸全無 Blocker**＝owner「每份 SRS 完成後多開 sub-agent 不同角度再驗」的落地（取代「單一 spec-reviewer」）。
| 軸 | 只看 | 對應易疏漏陷阱 |
|---|---|---|
...
> 〔QA flow 驗證軸（原 §4d）已移除——QA Agent Flow 暫不納入；先把 Bible→PRD→SRS→RD→DoD 跑順。**QA 產生/驗收 2026-06-24 亦一併暫拔除**：bundle 不再產 qa-cases、gate④/⑤（qa-cases/覆蓋率）暫停（`docs/specs/qa-to-test.md` 休眠）；code 階段驗證暫由機械閘門＋`spec-reviewer`＋Phase V runtime 自驗接。日後再決定 QA flow 何處介入，恢復見 git history。〕
...
- **T1（金錢/評分/授權）頁＝per-page 多 agent 驗證點**（owner 2026-06-25 改自「per-page 人審停點」）：code 產完即 spawn RD 軸（§4c）≥4 隻跨模型 read-only 驗 → Blocker 則**修該頁再驗** → 無 Blocker 即 rd-done、**drain 不停人、續下一頁**（不逐頁停人審）。**correlated-blindness 防護由 per-page 多 agent 跨模型驗承載**（非人審停點＝原 2026-06-22 規則的實質保留、只換交付者）；**人審收斂到批末 + owner Done**。批末另跑**批次多 agent 驗證**（跨頁一致性 + 跨批 regression + RD 軸對合併 diff）→ 再交人審/owner。最高風險頁仍可 owner 指定單跑。

> 〔QA 軌迴圈（原 §5d）已移除——QA Agent Flow 暫不納入。RD 後直接進 DoD 閘門牆（gate①②③⑥⑦；**gate④ qa-cases／⑤覆蓋率 隨 QA 產生/驗收 2026-06-24 暫拔除而暫停**），owner 蓋 `done`。待 Bible→PRD→SRS→RD→DoD 跑順後，再決定 QA flow（三層測試/報告）何處介入，恢復見 git history。〕
