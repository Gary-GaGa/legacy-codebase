...
## 0. 為什麼一定要分階段
- 舊 EPRO 系統（Java + ~250 JSP；DB＝**Oracle**——06-12 實連更正，原依 `DB2PoolSvc.xml` 誤判 DB2）規模大；「舊 + 新 + 比對」一次塞不進單一 context window。
...
> 一個 session 只讀舊系統**該頁**的 trx/action/DAO/SQL（**指名檔**），輸出一份**小 spec** 到 `docs/legacy-extract/<page>.md`：步驟 / 表 / 關鍵 SQL / 狀態轉移 / 方言殘留點（若有 DB2 語法）。**不比對、不碰新碼。**
>
...
| §2 撥貸 D1–D7 | **走 §3 三步**：A 抽舊 `0920/0921/0922` → A2 schema mapping（舊→新/整併）→ B 比新 `DataInput/Summary`；D2（T24 16 表）再拆 | 抽 ~3 + map 1 + 比 ~7 |
...
輸出到 docs/legacy-extract/<page>.md，含：① 每個動作的步驟 ② 讀寫的表/欄位 ③ 關鍵 SQL（殘留 DB2 語法則標出：FETCH FIRST/WITH UR 等；06-12 起舊庫＝Oracle）
