...
> 進行中（依序）：✅ #8 跨檔完整性 + 強制點欄檢查（`c470264`）；✅ #5 **partial-Approved 狀態**（`Approved (subset)`）；✅ #6 **`docs/pending-register.md`**（待決總表）；待做 #10 **Bible skill**、#4 **QA→可跑測試自動化**。openapi 兩源 reconcile + #12 git 紀律 → 維持人控/待議。
