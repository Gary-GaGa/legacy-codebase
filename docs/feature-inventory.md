...
| EPROC00119 | Financial Statement FI | ✅ | ✅ | ✅ F-8 已修（`6919da5`，options 接 `epl-sele-c0-financial-statement-comments`）；SRS in-review：`docs/specs/srs/EPROC00119/`（RP40-RP54 closed、fix-card 🟡 真修；**待補 PRD §8 兩條 Finished 阻擋（DIFFERENCE!=0、第一年度 Total Assets/Liab=0）→ 再審後蓋章**，見 pending-register）；**Phase V 必驗**：下拉有值+save 帶值+GI-sele 對 FI（businessType F）無分支影響；授權列；export 模板沿用 i0？|
