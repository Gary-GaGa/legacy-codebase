...
|---|---|:--:|:--:|---|
| EPROC00110 | 評分容器（corporate credit-investigation）| ✅ | ✅ | `epl-info/save-c0-credit-investigation-tab`（`CsuCreditInvestigationController`；Step 1 容器，BE 驅動動態 tab）；SRS in-review：`docs/specs/srs/EPROC00110/`（gate PASS；N 軸 2026-06-24 Should-fix closeout PASS：3🔴 已清，並補 R9 checkpoint 四併二互斥 provenance/db-diff 追溯、`EPROCSU0160` physical-but-excluded schema note；axis-A + cross-model no new Should-fix；**規格定版 Approved 2026-06-24**（owner stamp、axis A–F 獨立確認 Blocker 全清）；實作完成 pending RD）|
| EPROC00112 | CBC Banking Relationship | ✅ | ✅ | `epl-info/save-c0-cbc-banking-relationship`（`7e8aaf7`）；SRS in-review：`docs/specs/srs/EPROC00112/`（gate PASS；N 軸 0🔴；**規格定版 Approved 2026-06-24**（owner stamp、axis A–F 獨立確認：Bible RBAC tuple 403 等已補）；實作完成 pending RD）|
| EPROC00114 | Collateral Assessment | 🟡 | ✅ | `epl-sele/info/save-c0-collateral-assessment`（`62ec62f`；無 c0 calc→隱 Calc 鈕、分數 BE save 算）；audit F-7：calc handler 空 return——**Phase V 驗鈕真隱→可降回 ✅**；SRS in-review：`docs/specs/srs/EPROC00114/`（gate PASS；N 軸 2026-06-24 Should-fix closeout PASS：3🔴 已清，並清 stale standalone calc/rate auth wording、補 BR-014/TC-017 cross-module reset disclaimer 為 `EPROCS_0170/0270` owning；axis-A + cross-model no new Should-fix；**規格定版 Approved 2026-06-24**（owner stamp、axis A–F 獨立確認 Blocker 全清）；實作完成 pending RD）|
| EPROC00115 | Borrower Group Exposure | ✅ | ✅ | `epl-sele/info/save-c0-borrower-group-exposure`（Step 1 BGE pilot）；授權列；**規格定版 Approved 2026-06-25**（owner stamp；finalize 12 項 + fix-card 🟡 全真修、axis A–F 獨立再審 0 Blocker）；實作完成 pending RD；殘留非阻擋 🟡 見 pending-register |
| EPROC00116 | Financial Statement GI | ✅ | ✅ | `epl-*-c0-financial-statement-comments`（`524d8dc`；calc 保留、export POST-blob 兩邊一致、無 00640 式不符）；授權列；export 模板沿用 i0？；SRS in-review：`docs/specs/srs/EPROC00116/`（gate PASS；N 軸 2026-06-24 Should-fix closeout PASS：2🔴 已清，R7 改為 FE-blocks + to-be BE hardening pending，補 BR-016 KHR no-decimal R/QA、QA-020 required blank negative；axis-A + cross-model no new Should-fix；**規格定版 Approved 2026-06-24**（owner stamp、axis A–F 獨立確認 Blocker 全清）；實作完成 pending RD）|
| EPROC00117 | Financial Evaluation GI | ✅ | ✅ | **business-only ✅**（`b14ae05`，決策 B）：接 `epl-sele-c0-financial-list`、`epl-info/save-c0-financial-business`；未接 `-financial-staff`（cleanup）；SRS in-review：`docs/specs/srs/EPROC00117/`（RP26-RP38 closed；gate PASS；**規格定版 Approved 2026-06-24**（owner stamp、axis A–F 獨立確認：SaveRequest DSR 金額欄+fixed USD 已補）；實作完成 pending RD；企金 parity 補比另軌 pending-register:26）；授權列 |
...
| EPROC00119 | Financial Statement FI | ✅ | ✅ | ✅ F-8 已修（`6919da5`，options 接 `epl-sele-c0-financial-statement-comments`）；**規格定版 Approved 2026-06-25**（owner stamp；RP40-RP55 closed、fix-card 🟡 全真修，PRD §8 兩條 Finished 阻擋已補入 R7/REF-D5/DEC-RP55，axis A–F 獨立再審 0 Blocker）；實作完成 pending RD；**Phase V 必驗**：下拉有值+save 帶值+GI-sele 對 FI（businessType F）無分支影響；授權列；export 模板沿用 i0？|
| EPROC00120 | Financial Evaluation FI | ✅ | ✅ | **business-only ✅**（`6b084fb`，決策 B）：接 `epl-info/save-c0-financial-evaluation-table-fi`；i0 FI list 不消費→`getMenu()`回`of({})`；未接 `-staff-fi`（cleanup）；授權列；**規格定版 Approved 2026-06-25**（owner stamp；P-001..P-013 + fix-card 🟡 全真修、axis A–F 獨立再審 0 Blocker）；實作完成 pending RD；殘留非阻擋 🟡 見 pending-register |
