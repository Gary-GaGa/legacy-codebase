...
| `EPROC00118` | c0 Corporate Scorecard | ✅ | 缺 controller+DTO+service | 鏡像 i0 `CorporateScorecardController`。**人審定案（2026-06-05）**：算法**不在** `CsuCreditEval*`，唯一在 i0 `FunctionServiceImpl.funcGetRate` → calc **注入 `FunctionService`**（§6.1 唯一例外，勿複製/重寫算法）；shell/DTO/checkpoint 全自足。⚠️ **escalation**：CU-return checkpoint 衝突見下方清單。詳見 `build-tasks/EPROC00118-corporate-scorecard.md`|
...
### 觀察
- 30% 缺口（**2026-06-04 校正後**）：① **企金評分後端 controller**（鏡像 i0）—— `EPROC00115`/`00116`/`00119`/`00120` ✅完成，餘 **2**（`00117`/`00118`）② **撥貸 `0920`** 缺後端 ③ 前端 ~~7 頁~~ **基本完成**（僅 `EPROCSU0130` 收尾）。
- 🚩 **escalation（待 `CsuCreditEval*` owner 人審，2026-06-05）**：`EPROC00118` 之 CU-return checkpoint 缺陷——既有 `CsuCreditEvalAndCreditDecisionServiceImpl` Return(98) 硬編碼只清 `TB_CHECK_POINTS_CS`、無 CU 分流；§6.1 禁改既有 `Csu*`，故不在 00118 修。**待辦**：先 Codex 唯讀確認 CU（無擔企金）案件是否真會走到 `00118` checkpoint；若會→排既有 service 加 CU 分流之獨立修正（需授權破 §6.1「禁改 Csu*」），若不會→此非 bug、結案。
- 📋 **待整合驗證清單**（dev/uat 後端真資料時集中跑，build 階段不阻擋）：`EPROZ00610` CR 呈現/資料；`EPROC00115` 新 `epl-*-c0-*` endpoint 的 `TB_API_AUTH`/`TB_ROLE_TASK` 授權列（CS/CU 判斷已 code review 確認正確）；`EPROC00116`/`00119` 新 endpoint 授權列 + export 模板沿用 i0（`EPROI00116`/`EPROI00119`）是否需 c0 專屬；`EPROC00120` 新 endpoint 授權列；報表 `00620–00650` 呈現（`00640` scorecard export FE POST blob vs BE GET `@RequestBody` 介面不一致需對）。
