...
撥貸核心(A-1)     █████░░░░░░░░░░░░░░░░  ~25%   OQ-1~5 全裁(06-16 parity)→施工-ready；待 domain RD 補 stub
owner 裁定收口    ███████████░░░░░░░░░  ~55%   06-16 關 RP9/AUD-6/AUD-9/A-1 OQ；餘 AUD-1~4/7/8、RP8/11、E1/E2、BP、撥貸 group
```
...
| Phase D | 撥貸解鎖（A-1+T24+domain） | 🟠 A-1 OQ 全裁→施工-ready；T24/domain group 仍卡 owner（§三） |
| Phase E | c0 收尾 + 授權列套用 | ⏳ 等 ops 套 `c0-authz-sql` |
| Phase R | 暫緩 track（R2 報表/檔案 API） | ⏸ 待拍板 |

## 三、真缺口（剩的 coding）— 只有一個
**🟠 撥貸核心（A-1 `funcGetExchangeRate`）= 施工-ready**：authorize 換匯 throw-stub。施工規格已備（`a1-funcGetExchangeRate-spec.md`）；**OQ-1~5 全裁（06-16 owner：撥貸 outbound 參數先對齊舊系統 parity）**——OQ-1=`IdNo=OVSLXLON01`、OQ-3=映射 `EPROIS0921_UI_RAET_FIND_ERROR`、OQ-4=throw 勿回 null、OQ-5=G/H 讀 `EX_RATE_BUY`（已修 `581e717`）。**剩＝撥貸 domain RD 照規格補 stub（補 return＋移尾端 throw＋兩表交易一致；非 Codex 逕補）**。⚠️ 唯一復驗點＝新環境 T24 拒收 `IdNo=01`（schema-diff 證 02=新庫）。連帶坐實 G/H 欄名 bug 已修。
...
| 群 | 項 | owner |
|---|---|---|
| 🔴 撥貸 | 〔A-1 OQ 已 06-16 全裁→施工-ready〕撥貸 domain group（0921/0922/T24、檢核嚴格度、KHR、欄寬、async、M6 完工日） | T24／撥貸 domain／DBA |
