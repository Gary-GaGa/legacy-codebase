...
撥貸核心(A-1)     █████████░░░░░░░░░░░  ~45%   ✅ A-1 stub 實作(daae4c3,06-16,mvn綠)；剩 T24 正確性+批次層 B001-B008(待碼驗)+spec-conformance
...
**🟢 撥貸核心（A-1 `funcGetExchangeRate`）＝已實作（`daae4c3`，06-16，mvn clean package 綠）**：依 a1-spec OQ-1~5 裁定補 return＋移尾端 throw → authorize 換匯總開關打通（OQ-5 G/H 欄名 bug 另已修 `581e717`）。**剩撥貸真完成**：① **T24 正確性**（B-group 來源欄/格式，domain/T24-spec gated）② **批次層 B001–B008**（F-OWN-1，新查出未追蹤、狀態待碼驗；B005 匯率/B006 async/B007 SFTP 為下游交付關鍵）③ **A-1 spec-conformance 確認**（terse 回報未涵蓋：OQ-1 `IdNo=01`、OQ-3 錯誤映射、OQ-4 throw、兩表同交易——需碼驗/Phase V）④ 唯一復驗點＝新環境 T24 拒收 `IdNo=01` 則改 02。
