...
> 更新：里程碑後刷新；最後更新 **2026-06-17**（**撥貸 owner 決策牆清空**：A-1 conformance PASS／批次層 AUD-10／T24 照舊／A-5 幣別 keep；PRD→SRS 轉換層硬化批判輪2/3；**06-17 撥貸殘 domain A-4/M6/B-1/G·H 全裁照舊**；**B-group push `3d6f446` 回填**；**進度盤納入 PRD→SRS 規格產出軌（前置就緒／SRS bundle 0/67 待重產）**）。

## 一、總進度（一句話）
**程式遷移 ~80% 碼在；FE 補建全收口；🟢 撥貸核心 A-1 換匯 ✅ + 批次層 AUD-10 ✅ + T24 全裁「照舊系統規格」（06-16 owner）→ B-group parity fix 已 commit/push（`3d6f446`；金錢欄人審過，06-17）。**無 🔴 owner 決策擋主線**。撥貸剩＝T24 端到端/接收驗證（Phase V/UAT）〔殘 domain A-4/M6/B-1/KHR-G·H 已 06-17 全裁照舊→轉 Codex 執行〕。**規格產出（PRD→SRS）前置就緒**（轉換層硬化批判輪2/3＋Bible v1.1＋dispatch 卡）、**SRS bundle 0/67 待重產**（owner local 跑 Codex；risk-tier 企金線/撥貸/00800 先）。當前主戰場＝Phase V runtime 驗證 ＋ PRD→SRS 重產。其餘卡 owner 裁定（AUD-2/3/4/7/8/11、RP8/11、BP、E1/2）。**

```
程式遷移(碼在)   ████████████████░░░░  ~80%   133/166 audit 列（源 feature-inventory §1）
規格產出(PRD→SRS) ██░░░░░░░░░░░░░░░░░░  ~10%   前置就緒：轉換層硬化(批判輪2/3 gate)✅·Bible v1.1✅·local 對比輸入盤點定稿✅·dispatch 卡備齊✅；SRS bundle 0/67（00800 v0.9 封存待重產；risk-tier 企金線/撥貸/00800 先）
...
| **spec 層（PRD→SRS 重產）** | 新 Bible/PRD → SRS bundle（worked example 00800 重置；覆蓋 SSOT＝`per-page-reinventory-matrix.md`）| 🟡 **前置就緒→待重產**：轉換層硬化(批判輪2/3)✅·Bible v1.1✅·local 輸入定稿✅·dispatch 卡✅；**SRS bundle 0/67**、owner local 跑 Codex（§六）|
