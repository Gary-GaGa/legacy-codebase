...
> 讀序＝① `CLAUDE.md`（憲法）② **本檔**（一眼看進度/剩什麼/卡誰）③ `pending-register.md`（待決+owner）④ `build-tasks/` 各卡（**待派 prompt 在卡裡**）⑤ `decisions.md`（流水帳）。**下一步優先序見 §六**。〔**最近 session 接回卡＝`build-tasks/session-handoff-2026-06-20.md`**：spec 軌收尾＋三向驗證。**06-20 PM 更新**：00100/00118 owner-decision @PENDING 已全關（bundle 無 open pending）→ 殘 Approved-blocker＝contract 軸 RD/DBA 實作 gap；下一步＝RD/DBA 補 implementation/seed/測試 或掛 source 交 Codex 放大〕
...
> 更新：里程碑後刷新；最後更新 **2026-06-20**（**撥貸 owner 決策牆清空**：A-1 conformance PASS／批次層 AUD-10／T24 照舊／A-5 幣別 keep；PRD→SRS 轉換層硬化批判輪2/3；**06-17 撥貸殘 domain A-4/M6/B-1/G·H 全裁照舊**；**B-group push `3d6f446` 回填**；**進度盤納入 PRD→SRS 規格產出軌**；**06-18 EPROZ00100 SRS v0.2 已產（prd-to-srs→機械閘門 PASS→spec-reviewer 修畢、In Review）→ 覆蓋 0/67→1/67；其後同日 owner 全清重跑 00100/00118 → 覆蓋退回 0/67；最終 pipeline 母資料夾重產 00100+00118 → push 回 main、機械 PASS、ledger in-review、覆蓋 2/67〔跨模型 N 軸複驗 Blocker 已修(06-19 owner)、opus×2 採納後複審 PASS〕；**06-20 owner-decision @PENDING 全關（00100 `Z001~Z010`＋00118 `001~018`）、bundle 無 open pending、機械閘門兩頁 PASS（本環境 pyyaml 深驗補跑）；Approved 仍 held＝contract 軸 FE/BE 實作未追 to-be（RD/DBA implementation gap，不以改回現況掩蓋）；覆蓋仍 2/67；**spec-reviewer 複審兩頁無 Blocker、產 RD/DBA closeout 卡**；**06-20 RD 首批回報 8/13 code 落地（compile/ng build pass）→ 剩 A1/B1 `TB_API_AUTH` seed apply（DBA）＋B2/B3/B4 init/token/bridge（RD 下一張）＋測試證據 → contract 軸尚未全 PASS、未升 Approved**）。

## 一、總進度（一句話）
**程式遷移 ~80% 碼在；FE 補建全收口；🟢 撥貸核心 A-1 換匯 ✅ + 批次層 AUD-10 ✅ + T24 全裁「照舊系統規格」（06-16 owner）→ B-group parity fix 已 commit/push（`3d6f446`；金錢欄人審過，06-17）。**無 🔴 owner 決策擋主線**。撥貸剩＝T24 端到端/接收驗證（Phase V/UAT）〔殘 domain A-4/M6/B-1/KHR-G·H 已 06-17 全裁照舊→轉 Codex 執行〕。**規格產出（PRD→SRS）前置就緒**（轉換層硬化批判輪2/3＋Bible v1.1＋dispatch 卡）、**SRS bundle 2/67**（00100/00118 已重產 in-review；**跨模型 N 軸複驗 Blocker 已修(06-19 owner)、opus×2 採納後複審 PASS；06-20 owner-decision @PENDING 全關、bundle 無 open pending → Approved 殘 blocker 改為 contract 軸 RD/DBA 實作 gap（非裁定缺）**；risk-tier 企金線/撥貸/00800 先）。當前主戰場＝Phase V runtime 驗證 ＋ PRD→SRS 重產。其餘卡 owner 裁定（AUD-2/3/4/7/8/11、RP8/11、BP、E1/2）。**

```
程式遷移(碼在)   ████████████████░░░░  ~80%   133/166 audit 列（源 feature-inventory §1）
規格產出(PRD→SRS) ██░░░░░░░░░░░░░░░░░░  ~12%   前置就緒＋**orchestrator/N 軸/pilot 卡已建·pipeline 真頁驗證有效**；**SRS bundle 2/67**（00100/00118 已重產 in-review；跨模型 N 軸複驗 Blocker 已修(06-19 owner)、opus×2 採納後複審 PASS）；risk-tier 企金線/撥貸/00800 先
...
撥貸核心(A-1)     ███████████████░░░░░  ~78%   ✅ A-1+conformance、✅ 批次層 AUD-10、✅ T24 照舊、✅ A-5 幣別 keep(只USD/KHR)、✅ B-group parity fix 已 commit/push(`3d6f446`；金錢欄人審過)；剩端到端/T24 接收驗證〔殘 domain A-4/M6/B-1/G·H 06-17 全裁照舊→執行〕
...
| **spec 層（PRD→SRS 重產）** | 新 Bible/PRD → SRS bundle（worked example 00800 重置；覆蓋 SSOT＝`per-page-reinventory-matrix.md`）| 🟡 **前置就緒→待重產**：轉換層硬化(批判輪2/3)✅·Bible v1.1✅·local 輸入定稿✅·dispatch 卡✅；**SRS bundle 2/67**（00100/00118 已重產 in-review；跨模型 N 軸複驗 Blocker 已修(06-19 owner)、opus×2 採納後複審 PASS）、owner local 重產 Codex（§六）|
