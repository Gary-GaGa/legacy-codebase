...
> **🧭 執行順序（owner 2026-06-25，企金 9 包 110–120）**：① **9 包 RD → rd-done**（`build-tasks/rd-dispatch-c0-110-120.md`；per-page 多 agent 驗、drain 不停人、批末批次驗）→ ② **SRS spec 兩半轉換**（翻新 9 包成 Contract/Appendix + 補 4 包 covers-prd；plan「一檔兩半」Part B）→ ③ **Phase V runtime**（含 110–120 FE runtime smoke）。**FE runtime 不在 RD**（build+靜態多 agent 驗抓不到 runtime bug，RV-1/RV-2 前例）→ 歸 Phase V。三段序列、不交錯。
