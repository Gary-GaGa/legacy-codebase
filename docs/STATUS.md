...
> **🧭 執行順序（owner 2026-06-25，企金 9 包 110–120）**：① ✅ **9 包 RD 完成**（rd-done + owner Done）→ ② ✅ **SRS 兩半轉換 + digest 完成**（13/14 + 規範固化）→ ③ **Phase V runtime（全自動）← 現在**：L1 self-driving（env manager `tools/local-env.ps1`，已驗收）+ **L2 讀型 API↔DB conformance＝DoD gate ⑧（RD per-page；owner 2026-06-27：讀型 v1/v2 only）**（有 harness 的頁 rd-done 前 blocking；`§4c`；v1.1 已跑、RI-2 首例閉環；v2-c0 抓 3 真 bug 修復中）。**寫入 v3＝移出 RD、歸未來 QA Flow 整合 tier**（流程耦合）；**L3 FE 歸屬待定**（留 Phase V or QA Flow）。架構＝`build-tasks/phase-v-automation-plan.md`、分層＝`decisions.md`。
