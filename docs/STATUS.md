...
- 🔧 **待派（Codex，prompt 在各卡裡）——建議並行（彼此獨立）**：
  - **P1** ① ~~**T24 非幣別欄 batch-fix**~~ → **findings 落 main（`e557af4`）待 recon**（卡 `t24-bgroup-legacy-parity-fix.md`；金錢欄最嚴審）② **Phase V API 自驗 harness v1**（卡 `phase-v-api-selfverify-harness.md`；唯讀、零風險）∥ ops 套 `c0-authz-sql`（解 403）
  - **P2** ③ **get-body #3** reviseditem GET query（卡 `get-body-contract-sweep.md`；⚠️ 應早於 harness 測 00800 init-query）④ ~~**AUD-11** CU0160 碼驗~~ → **碼驗回 06-17＝UNFOUND·先不關**（無獨立 CU0160→傾向已併；殘 `TB_PAGE_MENU` SQL + `:597` ISU0160 分流→DBA/RD，見 §五/pending；findings Codex 樹待 push）
