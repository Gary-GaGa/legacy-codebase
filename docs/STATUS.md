...
> 🧭 **Model A（owner 2026-06-26）**：Bible/PRD/SRS **bundle 本體已移母資料夾、Codex 擁/產/驗**（本 repo 留範本/腳本/方法論/trackers；SRS 機械閘+spec-reviewer 在母資料夾跑）。**coverage/status SSOT＝`build-tasks/refactor-audit/per-page-reinventory-matrix.md` ledger**（下方 13/67 等數字為 ledger 衍生快照、非 bundle 直讀）。見 `decisions.md` Model A 列。
> **🧭 執行順序（owner 2026-06-25，企金 9 包 110–120）**：① ✅ **9 包 RD 完成**（rd-done + owner Done）→ ② ✅ **SRS 兩半轉換 + digest 完成**（13/14 + 規範固化）→ ③ **Phase V runtime（全自動）← 現在**：L1 self-driving（env manager `tools/local-env.ps1`，已驗收）+ **L2 API↔DB conformance＝升為 DoD gate ⑧、併入 RD per-page 迴圈**（有 harness 的頁 rd-done 前 blocking；`orchestration-playbook §4c`；harness v1.1 已跑、**RI-2 RD 修畢→owner Done 2026-06-26＝gate ⑧ 首例閉環**）+ **L3 FE Playwright**（取代人工 smoke）。**段③殘留＝跨頁整合 + L3 FE**（無法 per-page 者）；FE runtime 不在 RD（RV-1/RV-2 前例）→ 歸 Phase V。架構＝`build-tasks/phase-v-automation-plan.md`。
> **🧭 專案序（owner 2026-06-25）**：**A＝9 包三段（①②done、③ Phase V 進行中）完成後 → 專注 B：把其餘需 SRS 的頁全部產出**（撥貸 0920 / CSU 主流程 8 / SU0170 finalize / ISU·i0·z0 增量；orchestrator-drain 自動產兩半+digest）。**B gated on**：owner 提供 PRD（標 ledger prd-ready）+ **CSU 各頁先 `c0-legacy-parity-recheck` parity 餵入**（否則漏舊 cs/cu）。B 之後 → 各頁 RD / parity / AUD 裁 / 撥貸 T24-UAT / QA 重啟（見 §五、pending-register）。
> Last updated **2026-06-25**: corporate `EPROC00115`/`EPROC00119`/`EPROC00120` promoted to **規格定版 Approved** after fix-round 🟡 真修 + independent cross-model axis A–F re-review (0 Blocker, no regression), mechanical `check-srs-bundle` PASS. `EPROC00119` Round 2 specifically added PRD §8 Finished blockers in R7/REF-D5/DEC-RP55 before owner stamp. 9 corporate (EPROC00110–00120) 實作完成 rd-done + owner Done 2026-06-25; remaining Approved pending RD (QA suspended). SRS 規格定版 coverage now **13/67** (`EPROZ00100`, `EPROC00118`, `EPROISU0921`, `EPROISU0922`, `EPROZ00800` + corporate `EPROC00110/00112/00114/00115/00116/00117/00119/00120`). Earlier 2026-06-24: 00110/112/114/116/117 owner-stamped after axis A–F confirmation; 企金 parity 補比 (pending-register row 26) remains a separate track.
...
Phase V runtime  █████░░░░░░░░░░░░░░░░  ~25%   V-1 登入✅；RV-1/RV-2 兩類 bug 已收斂；**c0 授權列已套 2026-06-25（403 前置解除）**；smoke V-2~V-6 待跑
...
| Phase E | c0 收尾 + 授權列套用 | 🟢 **授權列 seed 已套+DBA 驗 OVSLXLON02 2026-06-25**（c0 403 前置解除；pxls==ppdf 驗畢；service-guard 層仍 code-stage）|
...
| smoke V-2~V-6（Phase G render/save/c0/dialog/teardown） | ⏳ 待跑（c0 授權列已套 2026-06-25，403 前置已解除）|
| **API 自驗 harness（=DoD gate ⑧ runtime conformance）** | ✅ **v1.1 已跑 2026-06-26**：LT-1~5+RI-1+RI-2 全 PASS、**gate ⑧ 首例閉環**（RI-2 owner Done）；起停改 env manager（`tools/local-env.ps1`）、三分類 sentinel；c0 v2/寫入 v3/FE-render(L3) 留另案 |
...
- 🔧 **待派（Codex，prompt 在各卡裡）——建議並行（彼此獨立）**：
  ...
  - **Wave 2（gated）**：
    ...
    - ⑤ **harness v2 + smoke V-4**（c0/csu 讀型/非 403）——c0-authz **已套+DBA 驗 OVSLXLON02 2026-06-25**（403 解除）；待跑 v2
...
- **ops**：~~`c0-authz-sql` 套 `OVSLXLON02`~~ ✅ **已套+DBA 驗 2026-06-25**（403 解除）；**B008** log 歸檔機制確認
