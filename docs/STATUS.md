...
> 2026-06-17 更新：T24 B-group parity fix 已套用並產出 `build-tasks/t24-bgroup-legacy-parity-fix-findings.md`；`E21`、`G4`/`G10`/`H8` 依 A-5 USD+KHR-only 邊界 keep。**金錢/截斷欄 pre-push 最嚴人審已過 → code 已 commit/push（product repo，06-17）**；剩端到端/T24 接收驗證（Phase V/UAT）。
...
## 一、總進度（一句話）
**程式遷移 ~80% 碼在；FE 補建全收口；🟢 撥貸核心 A-1 換匯 ✅ + 批次層 AUD-10 ✅ + T24 全裁「照舊系統規格」（06-16 owner）→ B-group parity fix 已 commit/push（金錢欄人審過，06-17）。**無 🔴 owner 決策擋主線**。撥貸剩＝T24 端到端/接收驗證（Phase V/UAT）+ 小 domain（A-4 檢核/M6）。當前主戰場＝Phase V runtime 驗證。其餘卡 owner 裁定（AUD-2/3/4/7/8/11、RP8/11、BP、E1/2）。**
...
撥貸核心(A-1)     ███████████████░░░░░  ~78%   ✅ A-1+conformance、✅ 批次層 AUD-10、✅ T24 照舊、✅ A-5 幣別 keep(只USD/KHR)、✅ B-group parity fix 已 commit/push(金錢欄人審過)；剩端到端/T24 接收驗證 + A-4檢核/M6
...
| Phase D | 撥貸解鎖（A-1+T24+domain） | 🟠 **owner 決策全清**（A-1 conformance PASS／批次層 AUD-10／T24 照舊／A-5 keep）→ **執行中**：T24 B-group parity fix 已 commit/push（金錢欄人審過），待端到端/T24 接收驗證（Phase V/UAT）；殘小 domain A-4/M6（§三）|
...
| 🟡 00800 | RP11（execute DTO）、RP8（R6/R7 判據）、**SR-B1/B2（06-16 語意審：R15+openapi 漏 PRD §6.4 兩錯誤碼／400-500 誤併，`build-tasks/00800-spec-review-findings.md`）**〔RP9 init-query＝GET 已 06-16 關（Follow PRD）〕| RD／架構 |
| 🟡 c0 | E1（CU-return checkpoint）、E2（crScoreCardCompleted 覆寫） | 信用決策 domain |
| 🟡 audit | AUD-2（0211/0213）、AUD-3（admin）、AUD-4（demo）、AUD-7（54 表）、AUD-8（權限表）、**AUD-11（CU0160 碼驗 06-17＝UNFOUND·先不關**：無獨立 CU0160→傾向已併，但卡 ① `TB_PAGE_MENU` data ② `CsuLoanConditionServiceImpl:597` 讀 ISU0160 分流存疑 → DBA(SQL)/RD(`:597`)〕〔AUD-1/6/9/**10** 已 06-16 關〕| PM／SA／DBA／domain／RD |
...
- 🔧 **待派（Codex，prompt 在各卡裡）——建議並行（彼此獨立）**：
  - **P1** ① **T24 B-group**：✅ code 已 commit/push（金錢/截斷欄人審過）→ 剩**端到端/T24 接收驗證**（Phase V/UAT）（findings：`t24-bgroup-legacy-parity-fix-findings.md`）② **Phase V API 自驗 harness v1**（卡 `phase-v-api-selfverify-harness.md`；唯讀、零風險）∥ ops 套 `c0-authz-sql`（解 403）
  - **P2** ③ **get-body #3** reviseditem GET query（卡 `get-body-contract-sweep.md`；⚠️ 應早於 harness 測 00800 init-query）④ ~~**AUD-11** CU0160 碼驗~~ → **碼驗回 06-17＝UNFOUND·先不關**（無獨立 CU0160→傾向已併；殘 `TB_PAGE_MENU` SQL + `:597` ISU0160 分流→DBA/RD，見 §五/pending；findings `done/aud11-cu0160-page-reverify-findings.md` 已 push `8c376f7`）
