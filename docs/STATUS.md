...
| Phase D | 撥貸解鎖（A-1+T24+domain） | 🟠 **code 決策清**（A-1 conformance PASS／批次層 AUD-10／T24 parity-fix `3d6f446`／A-5 keep）→ code 執行中：待端到端/T24 接收驗證（Phase V/UAT）；殘小 domain A-4/M6/B-1/G·H 06-17 裁照舊（code 層）；**⚠️ 06-20 T24/A-4/M6 SRS-層 re-open→產 SRS 走 §5b 梯裁+owner confirm（§三/banner）**|
