...
> 🧭 specs 在母資料夾（Codex；見 `decisions.md` Model A）→ **coverage/status SSOT＝`build-tasks/refactor-audit/per-page-reinventory-matrix.md` ledger**（下方 13/67 等＝ledger 衍生快照）。
> **🧭 執行順序（owner 2026-06-25，企金 9 包 110–120）**：① ✅ **9 包 RD 完成**（rd-done + owner Done）→ ② ✅ **SRS 兩半轉換 + digest 完成**（13/14 + 規範固化）→ ③ **Phase V runtime（全自動）← 現在**：L1 self-driving（env manager `tools/local-env.ps1`，已驗收）+ **L2 讀型 API↔DB conformance＝DoD gate ⑧（RD per-page；owner 2026-06-27：讀型 v1/v2 only）**（有 harness 的頁 rd-done 前 blocking；`§4c`；v1.1 + **v2-c0 17/17 PASS**〔langType/00117/00119+00120 seed 三缺口修畢〕＝**9 c0 頁讀型收尾**）。**寫入 v3 + FE L3＝移出 RD、歸未來 QA Flow 整合 tier**（流程/瀏覽器耦合）。→ RD 端 Phase V 讀型已收，**下一步＝B 段全 SRS**。分層＝`decisions.md`、runbook＝`build-tasks/phase-v-completion-runbook.md`。
