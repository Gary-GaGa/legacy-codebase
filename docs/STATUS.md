...
**🟢 撥貸核心（A-1 `funcGetExchangeRate`）＝已實作＋conformance PASS（`daae4c3`，06-16，mvn 綠）**：補 return＋移尾端 throw → authorize 換匯總開關打通；**Codex 唯讀碼驗 4/4 PASS**（OQ-1 `IdNo=OVSLXLON01`、OQ-3 非0000拋錯中止 authorize〔碼＝專屬 `FAILED_E304`〕、OQ-4 throw 勿回 null、兩表同 `@Transactional`）。OQ-5 G/H 欄名 bug 另已修 `581e717`。**剩撥貸端到端真完成**：① **T24 正確性**（B-group 來源欄/格式，domain/T24-spec gated）② **批次層 B001–B008**（F-OWN-1/AUD-10，新查出未追蹤、待碼驗；B005 匯率/B006 async/B007 SFTP 為下游交付關鍵）③ 唯一復驗點＝新環境 T24 拒收 `IdNo=01` 則改 02。
