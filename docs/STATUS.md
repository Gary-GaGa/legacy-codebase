...
> 2026-06-17 更新：T24 B-group parity fix 已套用並產出 `build-tasks/t24-bgroup-legacy-parity-fix-findings.md`；`E21`、`G4`/`G10`/`H8` 依 A-5 USD+KHR-only 邊界 keep。未 commit/push；金錢/截斷欄仍需 pre-push 最嚴人審與端到端/T24 接收驗證。
...
## 一、總進度（一句話）
**程式遷移 ~80% 碼在；FE 補建全收口；🟢 撥貸核心 A-1 換匯 ✅ + 批次層 AUD-10 ✅ + T24 全裁「照舊系統規格」（06-16 owner）→ B-group parity fix 已套用（06-17）。**無 🔴 owner 決策擋主線**。撥貸剩＝T24 pre-push 人審/端到端驗證 + 小 domain（A-4 檢核/M6）。當前主戰場＝Phase V runtime 驗證 + T24 review/驗證。其餘卡 owner 裁定（AUD-2/3/4/7/8/11、RP8/11、BP、E1/2）。**
...
撥貸核心(A-1)     ███████████████░░░░░  ~75%   ✅ A-1+conformance、✅ 批次層 AUD-10、✅ T24 照舊、✅ A-5 幣別 keep(只USD/KHR)、✅ B-group parity fix 已套用；剩 pre-push review/端到端驗證 + A-4檢核/M6
...
| Phase D | 撥貸解鎖（A-1+T24+domain） | 🟠 **owner 決策全清**（A-1 conformance PASS／批次層 AUD-10／T24 照舊／A-5 keep）→ **執行中**：T24 B-group parity fix 已套用，待 pre-push review + Phase V；殘小 domain A-4/M6（§三）|
...
- 🔧 **待派（Codex，prompt 在各卡裡）——建議並行（彼此獨立）**：
  - **P1** ① **T24 B-group pre-push review/端到端驗證**（findings：`t24-bgroup-legacy-parity-fix-findings.md`；金錢/截斷欄最嚴審）② **Phase V API 自驗 harness v1**（卡 `phase-v-api-selfverify-harness.md`；唯讀、零風險）∥ ops 套 `c0-authz-sql`（解 403）
