...
| 🟡 c0 | **c0 老系統 parity 補比（reopen 06-17，卡 `c0-legacy-parity-recheck.md`，risk-tier 00118/00120 先）**；E1（CU-return）、E2（crScoreCardCompleted 覆寫）→ 卡 `c0-crediteval-e1-e2-escalation.md`（與 T1 00118 合流）| 信用決策 domain |
...
- 📐 **重新盤點驅動（選 A，06-17；非 wholesale 重建）**：per-page 矩陣 `refactor-audit/per-page-reinventory-matrix.md`——每頁套 SOP 三判定 disposition；確定 rebuild 僅 00800、最大 parity 風險＝企金線 18 頁（Wave1 ④）；**新版 Bible/PRD → SRS**（owner 提供放進 `docs/specs/` 後按 risk-tier 跑 `prd-to-srs`：企金線/撥貸/00800 重產先）
