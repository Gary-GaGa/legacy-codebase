...
> 2026-06-17 執行回填：`build-tasks/t24-bgroup-legacy-parity-fix-findings.md` 已逐欄坐實舊 `file:line`，B-2、B-3 非幣別欄、B-4、B-5 修正已套用；`E21`、`G4`/`G10`/`H8` 依 A-5 owner keep，不做對等修。金錢/截斷欄仍需 pre-push 人審與後端完整 package gate。
...
| **B-2** ✅ 修正已套用（06-17） | `A16` `NORMAL.LAON`→`LOAN` | 坐實舊 T24 key 為 `NORMAL.LAON`，新碼已改回；待 pre-push review | 改錯會破 T24·中 | findings |
...
| **B-4** ✅ 修正已套用（06-17） | `AGREEMENT_NO` 截斷（`A31`/`G7`，舊取後 16 碼） | `A31`/`G7` 已照舊取後 16 碼；待金錢/截斷欄 pre-push review | T24 欄寬·中 | findings |
