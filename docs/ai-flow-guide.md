...
> **這份是什麼**：把 AI-native SDLC pipeline（Legacy→Bible→PRD→SRS→QA→RD→DoD 閘門）串成**一條由淺入深的閱讀線**，給新進團隊冷讀、也給單兵交接用。
...
1. **funcId 追溯骨幹**：一個 slug（例 `EPROC00118`）串起 `Bible → PRD REQ-nnn → SRS Rn → QA case → code/test`。原則：**↑可追溯、↓可驗證**。任何一段拿出來，都能往上找到「為什麼要做」、往下找到「怎麼驗它做對」。
...
| ④ | **QA** | SRS（Approved） | QA-Agent | `qa-cases.md`（covers Rn·確定性 oracle） | 輸入閘：Rn 沒 acceptance/強制點 → **退回 SRS** | bundle 內 `qa-cases.md` |
| ⑤ | **RD** | SRS ＋ QA | RD-Agent（Codex，母資料夾） | 產品碼（**repo 外**） | 有 migrated 碼→as-is 驗證；無→greenfield 先盤點等審 | in-repo＝任務單 `build-tasks/*.md` |
| ⑥ | **DoD 閘門牆** | code | 7 道閘 | ✅ Done | ①契約 ②schema ③verify-c0 ④QA驗收 ⑤覆蓋率 ⑥build ⑦LLM審(advisory) | `scripts/` ＋ `verification/*` |
...
| `qa-cases.md` | 每條 `covers: Rn` | 連回 spec |
...
2. **語意層**：**SRS N 軸驗證**（七正交軸 A–G，見 [`orchestration-playbook §4b`](process/orchestration-playbook.md)）——A 綜合完整性（＝[`spec-reviewer`](../.claude/agents/spec-reviewer.md)）、B as-is parity、C 錯誤碼承載、D 安全授權、E DB reconcile、F 金錢/精度/截斷、G 可測試性。**各軸 read-only、獨立 session、最好跨模型**（反 correlated-blindness）。
...
- **DoD 細目**：見 [`prd-to-srs`](../.claude/skills/prd-to-srs/) skill §DoD（每 REQ≥1 Rn、每 Rn 有 acceptance+≥1 QA covers+強制點、happy/error/edge、每 TBD 一條 @PENDING+owner、Traceability Matrix 完整…）。

### ④ SRS → QA — QA-Agent：文件變成「可判真假的 oracle」

> QA 的軌性格：活語料（一條 Rn 可生 1~∞ case）、**會反壓上游**。

子流程（`ai-workflow.mmd` 的 `QAA` subgraph）：
1. **輸入閘**：Rn 有 acceptance＋強制點？openapi/schema 在？**否 → 退回 SRS**（不替 SA 補意圖）。
2. **拆 case**：每條 Rn → happy / error / edge。
3. **oracle 化**：given · when · then · 判官（確定性）。**error 由 openapi 錯誤碼反推**（一碼 ≥ 一 case）；**edge 溯 schema 約束/量化詞**（非湊數）。
4. **強制點施測層**：BE-enforced 的 Rn → 額外「**繞過 FE 直打端點**」的 negative case（＝N 軸 axis D）。
5. **產 `qa-cases.md`**：covers Rn、確定性 oracle。

- **鐵律**：**oracle 寫不出 ＝ SRS 漏**（Rn 互斥/缺意圖）→ 反壓退回，**不硬湊 coverage theater**。
- **橋接到測試**：QA case 怎麼變可跑測試（Testcontainers oracle-xe）＝`specs/qa-to-test.md`，對應閘門 ④。
- **QA Agent Flow（編排化，code→test）**：cases →【三層測試 DB(Testcontainers)+BE(epl-*)+FE(Playwright)】→ 跑 → **測試報告清單**。權威迴圈＝[`orchestration-playbook §5d/§6d`](process/orchestration-playbook.md)；運行殼＝`build-tasks/qa-orchestrator-drain.md`、單頁＝`qa-codex-dispatch.md`；FE＝`specs/fe-test-convention.md`、報告＝`specs/qa-report-format.md`。QA 軸＝`§4d`。首跑 pilot＝`EPROC00118`。
...
- **RD Agent Flow（編排化，SRS→code）**：權威迴圈＝[`orchestration-playbook §5c/§6c`](process/orchestration-playbook.md)；運行殼＝`build-tasks/rd-orchestrator-drain.md`、單頁＝`rd-codex-dispatch.md`；RD 軸＝`§4c`（contract/scope/regression/強制點落實）。終點＝`rd-done`（交 QA flow，不自宣 Done）。

### ⑥ DoD 閘門牆 — RD 通過 SA/QA 邊界

七道閘（`ai-workflow.mmd` 的 `GATE` subgraph）：
...
| ④ QA 驗收 | 跑三層測試 DB(Testcontainers)+BE(epl-*)+FE(Playwright)＝QA flow（`§5d`） | deterministic |
| ⑤ 覆蓋率 | ID 對表（每 Rn 有 covers） | deterministic |
...
- ①–⑥ 全綠 → **✅ Done**；任一 FAIL → 回 RD 修、重跑。

### 回流路徑（flow 不是直線）

- **Done → Issue → 回歸**：上線後 bug → 新增 QA case（回歸，1~∞）回到 ④ 的「拆 case」。
...
> 把上面抽象的 ①→⑥ 接到一個**已 Approved**（規格＋實作，RD/DBA closeout 2026-06-20）的真實 bundle，新人最有感。bundle＝[`specs/srs/EPROC00118/`](specs/srs/EPROC00118/)。

**全程一覽**

| 站 | 這頁實際發生什麼 | 證據 |
|---|---|---|
| ① Bible | 企金 C0 scorecard 的業務意圖（評分→風險等級→影響授信決策） | `specs/bible/bible-eproposal.md:398-399, 739-744` |
| ② PRD | 收斂成 `FR-001`–`FR-008`（初始化/項目/Rate計算/Default/AO·CR控制/Save/checkpoint/精度） | `PRD-CDC-EPRO-0001-EPROC00118-v1.0.md` |
| ③ SRS | `R1`–`R8`（各標 `covers-prd` 與強制點）＋ bundle 四檔；4 個真實端點 `epl-{sele,info,calc,save}-c0-corporateScorecard` | `EPROC00118/spec.md` |
| ④ QA | 每條 Rn 拆 happy/error/edge，covers 指回 Rn | `EPROC00118/qa-cases.md` |
...
- **④ QA 由契約反推**（一碼≥一 case、edge 溯 schema）：
  | QA | 類型 | 測什麼 | 來源 |
  |---|---|---|---|
...
> 一句話總結這條鏈：**funcId 串起 `FR-003 → R3 → QA-010/011A/012/013`**，↑每一步都追得到「為什麼」、↓每一步都有 case 驗「做對沒」，判斷題全部 owner 拍板、brownfield drift 經三判才處置。
...
- **SRS 軌 drain**：PRD→SRS 可批量，但**序列一次一頁**、每頁全 A–G 跨模型、終點 `in-review`（不自升 approved）；T1（金錢/授權/計分）頁每頁停交人審、不併批末。
...
5. 看本檔 **L2½ 的 `EPROC00118` 全程範例**，再挑一個**已 Approved 的 SRS bundle** 親自精讀（`specs/srs/<funcId>/` 四檔，如 `EPROC00118`/`EPROZ00100`）— 把抽象變具體。
...
| 詞 | 意思 |
|---|---|
| **funcId** | 追溯 slug（例 `EPROC00118`），串 Bible→PRD→SRS→QA→code/test |
...
| **covers** | QA case 以 `covers: Rn` 指回它測的規格 |
...
| **oracle** | QA case 的「判官」——given/when/then 可確定性判真假 |
| **@PENDING** | 待人裁的判斷題，附 owner＋是否 blocking，聚合到 pending-register |
| **N 軸（A–G）** | SRS 階段七正交語意驗證軸（blocking）；A＝spec-reviewer |
