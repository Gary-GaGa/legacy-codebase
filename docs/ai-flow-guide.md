...
| ③ SRS | `R1`–`R8`（各標 `covers-prd` 與強制點）＋ bundle 三檔；4 個真實端點 `epl-{sele,info,calc,save}-c0-corporateScorecard` | `EPROC00118/spec.md` |
...
5. 看本檔 **L2½ 的 `EPROC00118` 全程範例**，再挑一個**已 Approved 的 SRS bundle** 親自精讀（`specs/srs/<funcId>/` 三檔，如 `EPROC00118`/`EPROZ00100`）— 把抽象變具體。
