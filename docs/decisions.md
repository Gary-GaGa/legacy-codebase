...
- 新 DB schema（Excel HOME，~71 表，表名同舊但**無 `EPRO_` 前綴、無 schema 限定**）：解 **B4**（`TB_APP_NO_SEQ`=APPLICATION_NO 序號）、shell 來源（`TB_PAGE_MENU`=pageMap）、**R7**（`TB_FUNCTION_AUTH`/`TB_API_AUTH`/`TB_ROLE_TASK`/`TB_ROLE_DEFINE` 已建表）；checkpoint 改名 `TB_CHECK_POINTS_IS/IU/CS/CU`。⚠️ **缺口**：CBC/財報/財務評估/Scorecard 表未在 HOME → 待確認 Excel 範圍。詳見 `db-schema-catalog.md`。
