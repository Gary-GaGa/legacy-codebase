...
| 批次層 AUD-10 碼驗部分回（2026-06-16, Codex 唯讀）| **新系統批次重編號≠legacy 號**（職責對映）。✅ **撥貸下游交付不缺**：B006 結果檔回寫＝新 `EPROZ0B006`(active scheduled)、B007 SFTP＝新 `EPROZ0B004`(排程下載/刪)＋authorize inline 上傳 T24。🔴 **B005 匯率排程＝唯一 app 碼缺**：新只有 inline 換匯（A-1 `funcGetExchangeRate`），無 scheduled 等價物（新 `EPROZ0B005`＝mail sender、`@Scheduled` 註解掉、不能抵）→ **待 domain 判「inline 取代每日批次 vs 漏建」，判準＝有無非-authorize 路徑讀當日 `TB_EXCHANGE_RATE`**。⚪ B008 log 歸檔 UNFOUND→ops（logrotate/平台）。❓ B001–B004 待完整 findings（B001 連動 B-1 T24_COMPANY）。findings＝`aud10-batch-layer-reverify-findings.md`（摘要留底；Codex 工作樹原檔含 file:line）。AUD-10 未結。 |
