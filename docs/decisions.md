...
| Model A：spec 本體移母資料夾、Codex 擁（owner 2026-06-26）| **Bible/PRD/SRS bundle 本體自本(規劃)repo 移除、改由 Codex 在母資料夾擁/產/驗**（本 repo 不再留 spec bundle、不再常態同步）。本 repo 保留：範本（`specs/srs/spec-template.md`/`digest-template.md`）、機械閘腳本（`scripts/check-srs-bundle.py`/`verify-c0.py`＝canonical 源、部署母資料夾跑）、方法論（`spec-architecture`/SKILL/`orchestration-playbook`）、治理、trackers。**取捨**：放棄 Claude 側 SRS 機械閘/`spec-reviewer`/交叉複審（specs 不在此）；換 Codex 母資料夾 hooks 跑機械閘（已驗：14 bundle `check-srs-bundle --all` exit 0、`.codex/hooks.json` 就位、spec-reviewer 跑過）。**現狀 SSOT＝`build-tasks/refactor-audit/per-page-reinventory-matrix.md` ledger**（coverage/status 不再靠 bundle）；本 repo Stop-hook 改驗 parity。spec 需複查時臨時拉特定 bundle。|
