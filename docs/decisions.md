...
| 批次層 AUD-10 碼驗完整回（2026-06-16, Codex 唯讀；findings 完整版已上 main `23aff35`）| **新系統批次重編號≠legacy 號**（職責對映）。**6/8 FOUND**（app 碼存在，全在新 `TaskController` + `EPROZ0B00x ServiceImpl`，`@Scheduled`）：B001 branch→新B001、B002 emp→新B002、**B003 自動結案→新B007**、**B004 暫存清理→新B003**、B006 結果檔→新B006、**B007 SFTP→新B004**＋authorize inline 上傳 T24。🔴 **B005 匯率排程＝唯一 app 碼缺**：新只有 inline 換匯（A-1 `funcGetExchangeRate`），無 scheduled 等價物（新 `EPROZ0B005`＝mail sender、`@Scheduled` 註解掉、不能抵）→ **待 domain 判「inline 取代每日批次 vs 漏建」，判準＝有無非-authorize 路徑讀當日 `TB_EXCHANGE_RATE`**。⚪ **B008 log 歸檔 UNFOUND→ops**（logrotate/平台）。findings＝`aud10-batch-layer-reverify-findings.md`（Codex 完整版，含 file:line + 新後端批次入口盤點）。**AUD-10 剩 B005 判定 + B008 ops 確認**。連帶：B001 FOUND → B-1 `T24_COMPANY` 有 branch import 資料路徑。 |
