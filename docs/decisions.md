...
| AUD-11 `EPROCU0160` vs `EPROCSU0160` 併頁裁決（owner 2026-06-28）| 新版 owner Excel `function-review.xlsx` / `查核總表` 以 merged-cell 視覺 target 為準：`E137:E140` 全歸 `EPROCSU0160`，涵蓋 `EPROCS_0160`/`0260` + `EPROCU_0160`/`0260`；`EPROCU0160` 不是新 target。三判＝**(b) 刻意演進·縮編**（CS/CU→CSU keep 新）＋checkpoint/schema 名稱層屬 **(c) DB 結構差**（CS/CU checkpoint table 均用 `EPROCSU0160` 欄，schema 以新為準）。因此 AUD-11 的「併 vs 分」關案；`feature-inventory.md §1A/§2B` 維持 `EPROCSU0160`。但 `CsuLoanConditionServiceImpl:597` 讀 `EPROISU0160` 仍是獨立 checkpoint potential regression，併入企金線 parity 補比處理，不被本裁決自動視為正確。|
