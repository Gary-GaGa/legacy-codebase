...
| 執行策略 | 重構系統實際為**前後端兩個獨立專案**（不上 GitHub）、**已完成 ~70%**；目標 = **補完剩 30%**（gap 分析驅動，非從零）。原始碼不外流 → 本 repo 只放規格/backlog，實作在各專案內(Copilot/本機)。工作單位 = **新系統頁**（重構小組將多舊頁合併為一新頁）。對應與 backlog 見 `page-mapping.md` |
